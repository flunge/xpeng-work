...
从源文档中提取并更新每个人的人物画像（[[personality-profiles]]）：

| 来源 | 提取什么 |
|------|---------|
| 逐字稿 | 发言内容、语气、追问模式、主动/被动、被谁cue |
| 智能纪要 | 被分配到什么待办、承担什么角色 |
| IM 消息 | 谁@了谁、回复速度、表达风格、群内参与度 |
| 文档 | 谁写了什么文档、什么风格 |

**每个在岗组员 + 高炳涛，每次同步至少检查是否有新观察可写入 `members/*.md`。**

#### Phase 5: 记忆写入

更新以下 memory 文件：
- `current-initiatives.md` — 项目进展、里程碑变化、风险
- `members/<name>.md` — 每个人的工作进展 + 行为画像更新
- `chat-insights.md` — 群聊关键决策、新事件
- `projects/*.md` — 如有关联项目文件
