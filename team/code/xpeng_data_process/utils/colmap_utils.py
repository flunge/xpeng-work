...
def read_points3D_text_as_points3D(path):
    """
    see: src/base/reconstruction.cc
...
def read_points3D_binary_as_points3D(path_to_model_file):
    """
    see: src/base/reconstruction.cc
...
def read_model(path, ext=""):
    ...
    if ext == ".txt":
        ...
        points3D = read_points3D_text_as_points3D(os.path.join(path, "points3D") + ext)
    else:
        cameras = read_cameras_binary(os.path.join(path, "cameras" + ext))
        images = read_images_binary(os.path.join(path, "images" + ext))
        points3D = read_points3D_binary_as_points3D(os.path.join(path, "points3D") + ext)
