...
def pq_pose_to_4x4(pq_pose):
    p = pq_pose["p"]
    q = pq_pose["q"]
    angles = [q["w"], q["x"], q["y"], q["z"]]
    pose_4x4 = quaternion_matrix(angles)
    translation = [p[x] for x in p]
...
def get_ecef2enu():
    ref_lle = np.array([23.029396697983, 113.158963807152, -3.721244164743])
    ref_ecef = np.array([-2309688.245991358, 5399583.691095771, 2479714.3942346666])
    ref_lat = ref_lle[0] * np.pi / 180.0
    ref_lon = ref_lle[1] * np.pi / 180.0
    ref_enu_R = np.array([[-np.sin(ref_lon), np.cos(ref_lon), 0],
                          [-np.sin(ref_lat) * np.cos(ref_lon), -np.sin(ref_lat) * np.sin(ref_lon),
                           np.cos(ref_lat)],
                          [np.cos(ref_lat) * np.cos(ref_lon),
                           np.cos(ref_lat) * np.sin(ref_lon),
                           np.sin(ref_lat)]])
    ecef2enu = np.zeros((4, 4))
    ecef2enu[0:3, 0:3] = ref_enu_R
    ecef2enu[0:3, 3] = -ref_enu_R @ ref_ecef
    ecef2enu[3, 3] = 1
