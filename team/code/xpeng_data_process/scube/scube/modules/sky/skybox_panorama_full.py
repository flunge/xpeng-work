...
class SkyboxPanoramaFull(nn.Module):
    def __init__(self, hparams):
        ...
        self.skybox_forward_sky_only = True
    ...
    def encode_sky_feature(self, batch, imgenc_output):
        ...
        B, N, C_mask, H_in, W_in = sky_mask.shape           # B=1, N=8, C_mask=1
        sky_mask_4d = sky_mask.view(B * N, C_mask, H_in, W_in)   # [8, 1, 928, 1920]
        sky_mask_resized_4d = F.interpolate(
            sky_mask_4d,
            size=(464, 960),
            mode='bilinear',
            align_corners=False
        )   # 输出 [8, 1, 464, 960]
        sky_mask_resized = sky_mask_resized_4d.view(B, N, C_mask, 464, 960)
        image_or_feature = image_or_feature * sky_mask_resized
        ...
        for bidx in range(B):
            ...
            # sampled_feature = torch.median(sampled_feature, dim=0)[0]
            sampled_feature = torch.topk(sampled_feature, k=2, dim=0).values[1]

            # 转成 [1, C, H', W'] 格式给 avg_pool2d 使用
            sampled_feature = sampled_feature.permute(2, 0, 1).unsqueeze(0)  # [1, C, H', W']

            # 同尺寸平均池化（stride=1，适当 padding 保持尺寸不变）
            kernel_size = 3   # 可以改成 5、7 等，根据你想要的平滑程度
            padding = kernel_size // 2  # 保持输出尺寸不变的关键

            sampled_feature = F.avg_pool2d(
                sampled_feature,
                kernel_size=kernel_size,
                stride=1,
                padding=padding,
                count_include_pad=False   # 可选：不把 padding 区域算进平均（更干净）
            )

            # 还原维度回 [H', W', C]
            sampled_feature = sampled_feature.squeeze(0).permute(1, 2, 0)
            panorama_mask = torch.max(panorama_mask, dim=0)[0] > 0 # [H', W', 1]
            # panorama_mask = (torch.median(panorama_mask, dim=0)[0] > 0).float()
    ...
    def forward(self, skyenc_output, network_output):
        ...
        # self.save_skybox(network_output, "./debug")
