...
class BaseModel(pl.LightningModule):
    def __init__(self, hparams):
        ...
        # self.local_sampled_set = {}
        # self.deleted_cases = set()
        # self.online_data_folder = hparams.online_data_folder
    ...
    #     print("---------------process on_train_batch_end--------------- ", batch_idx)

    #     case_id_list = batch.get('case_id')
    #     # frame_cam = batch.get('frame_cam')
    #     # print("case_id ", case_id)
    #     # print("frame_cam ", frame_cam)
    #     # if case_id and frame_cam:
    #     #     if isinstance(frame_cam, (list, tuple)):
    #     #         frame_cam = tuple(frame_cam)
    #     #     self.local_sampled_set[case_id].add(frame_cam)

    #     print("-----self.global_rank----- ", self.global_rank)
    #     for case_id in case_id_list:
    #         print("=====case_id==== ", case_id)
    #         if case_id not in self.local_sampled_set.keys():
    #             self.local_sampled_set[case_id] = 1
    #         else:
    #             self.local_sampled_set[case_id] += 1

    #     print("-----self.local_sampled_set------ ", self.local_sampled_set)
    #     self.sync_and_process_deletions(batch['required'])
    #     return
    
    # def sync_and_process_deletions(self, total_required_num):
    #     if not dist.is_available() or not dist.is_initialized():
    #         self.process_local_deletions(total_required_num)
    #     else:
    #         self.process_multi_gpu_deletions(total_required_num)

    # def process_local_deletions(self, total_required_num):
    #     completed_cases = []
    #     for case_id, counts in self.local_sampled_set.items():
    #         if counts >= total_required_num * 0.85:
    #             completed_cases.append(case_id)

    #     for case_id in completed_cases:
    #         if case_id not in self.deleted_cases:
    #             case_folder = os.path.join(self.online_data_folder, case_id)
    #             if os.path.exists(case_folder):
    #                 # shutil.rmtree(case_folder)
    #                 print(f"============Deleted case folder: {case_folder}============")
    #             self.deleted_cases.add(case_id)
    #     self.local_sampled_set.clear()

    # def process_multi_gpu_deletions(self, total_required_num):
    #     local_sampled = dict(self.local_sampled_set)
    #     all_local_sampled = [None] * self.trainer.world_size
    #     dist.all_gather_object(all_local_sampled, local_sampled)

    #     if self.global_rank == 0:
    #         print("all_local_sampled ", all_local_sampled)
    #         global_sampled = {}
    #         for rank_sampled in all_local_sampled:
    #             for case_id, sampled_counts in rank_sampled.items():
    #                 if case_id not in global_sampled.keys():
    #                     global_sampled[case_id] = sampled_counts
    #                 else:
    #                     global_sampled[case_id] += sampled_counts

    #         print("global_sampled ", global_sampled)

    #         completed_cases = []
    #         for case_id, sampled_counts in global_sampled.items():
    #             if sampled_counts >= total_required_num * 0.85:
    #                 completed_cases.append(case_id)
    ...
    #                         print(f"============ Failed to delete {case_folder}: {e}============")
    #                 self.deleted_cases.add(case_id)

    #     dist.barrier()

    #     all_deleted = [None] * self.trainer.world_size
    #     dist.all_gather_object(all_deleted, list(self.deleted_cases))
    #     global_deleted = set()
    #     for s in all_deleted:
    #         global_deleted.update(s)
    #     self.deleted_cases = global_deleted

    #     self.local_sampled_set = {
    #             k: v for k, v in self.local_sampled_set.items() if k not in self.deleted_cases
    #         }
    #     dist.barrier()
    #     return
    ...
    def train_dataloader(self):
        ...
        return DataLoader(train_set, batch_size=self.hparams.batch_size // self.trainer.world_size, 
                          shuffle=False if not isinstance(train_set, torch.utils.data.IterableDataset) else False,
