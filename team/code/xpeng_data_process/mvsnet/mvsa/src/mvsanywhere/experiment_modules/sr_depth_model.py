...
from mvsnet.mvsa.src.mvsanywhere.losses import (
    MSGradientLoss,
    # MVDepthLoss,
    NormalsLoss,
    ScaleInvariantLoss,
)
from mvsnet.mvsa.src.mvsanywhere.modules.cost_volume import CostVolumeManager, FastCostVolumeManager
from mvsnet.mvsa.src.mvsanywhere.modules.depth_anything_blocks import DPTHead
from mvsnet.mvsa.src.mvsanywhere.modules.feature_volume import FastFeatureVolumeManager, FeatureVolumeManager
from mvsnet.mvsa.src.mvsanywhere.modules.view_agnostic_feature_volume import FastViewAgnosticFeatureVolumeManager, ViewAgnosticFeatureVolumeManager
from mvsnet.mvsa.src.mvsanywhere.modules.layers import TensorFormatter
from mvsnet.mvsa.src.mvsanywhere.modules.networks import (
    CVEncoder,
    DepthDecoderPP,
    ResnetMatchingEncoder,
    UNetMatchingEncoder,
)
from mvsnet.mvsa.src.mvsanywhere.modules.networks_fast import SkipDecoderRegression
from mvsnet.mvsa.src.mvsanywhere.modules.vit_modules import DINOv2, ViTCVEncoder
from mvsnet.mvsa.src.mvsanywhere.utils.augmentation_utils import CustomColorJitter
from mvsnet.mvsa.src.mvsanywhere.utils.generic_utils import (
    reverse_imagenet_normalize,
    tensor_B_to_bM,
    tensor_bM_to_B,
)
# from mvsnet.mvsa.src.mvsanywhere.utils.geometry_utils import NormalGenerator
from mvsnet.mvsa.src.mvsanywhere.utils.metrics_utils import compute_depth_metrics
from mvsnet.mvsa.src.mvsanywhere.utils.visualization_utils import colormap_image
...
class DepthModel(pl.LightningModule):
    ...
    def get_heuristic_depth_range(self, cur_data, src_data):
        ...
        cam_dists_min_per_batch = torch.amin(cam_dists, 1)  # B
        cam_dists_max_per_batch = torch.amax(cam_dists, 1)  # B
        src_K_focal = src_K[:, 0, 0, 0]  # B
        img_width = cur_image.shape[-1]
        
        zero_cam_dists = (cam_dists_min_per_batch == 0).any()
        zero_focal = (src_K_focal == 0).any()
        
        if zero_cam_dists or zero_focal:
            print(
                f"DepthModel.get_heuristic_depth_range: Found zero values! "
                f"zero_cam_dists={zero_cam_dists.item() if zero_cam_dists else False}, "
                f"zero_focal={zero_focal.item() if zero_focal else False}, "
                f"cam_dists_min_per_batch: min={cam_dists_min_per_batch.min().item():.6f}, "
                f"src_K_focal: min={src_K_focal.min().item():.6f}"
            )
        
        baseline_min_depth = (cam_dists_min_per_batch * src_K_focal) * K_scale / (1.0 * img_width)
        baseline_max_depth = (cam_dists_max_per_batch * src_K_focal * K_scale) / (0.01 * img_width)
        
        min_depth_safe = torch.clamp(baseline_min_depth, min=0.1)  # Ensure at least 0.1
        max_depth_safe = torch.clamp(baseline_max_depth, min=min_depth_safe + 0.1)  # Ensure max > min
        
        return min_depth_safe, max_depth_safe
    ...
    def forward(
        ...
        Returns:
            depth_outputs: a dictionary with outputs including
                ...
                "overall_mask_bhw" returned when return_mask is True and
                    is a 2D mask from the cost volume for areas where there
                    is source view information from the current view's point
                    of view.
                "confidence_bhw" photometric confidence measure (BxHxW):
                    averaging probability over 4 neighboring depth planes
                    at the predicted depth location. Higher values indicate higher confidence
                    in the depth prediction with smoother and more robust estimates.
        ...
        ### TODO(zf): check new feature ####################################################
        # Compute photometric confidence
        # Use 4 neighboring depth planes' average probability
        with torch.no_grad():
            # Convert cost volume to probability volume
            prob_volume = F.softmax(cost_volume.detach(), dim=1)  # B x D x H x W
            
            # Average over 4 neighboring depth planes
            # Pad depth dimension: [1, 2] means pad 1 at start, 2 at end
            prob_volume_sum4 = 4 * F.avg_pool3d(
                F.pad(prob_volume.unsqueeze(1), pad=(0, 0, 0, 0, 1, 2)),
                kernel_size=(4, 1, 1),
                stride=1,
                padding=0
            ).squeeze(1)  # B x D x H x W
            
            # Get depth index from lowest_cost
            num_depth = prob_volume.shape[1]
            # Normalize lowest_cost to depth index range
            depth_min = min_depth.view(-1, 1, 1)
            depth_max = max_depth.view(-1, 1, 1)
            depth_index = ((lowest_cost - depth_min) / (depth_max - depth_min + 1e-6) * (num_depth - 1))
            depth_index = depth_index.clamp(min=0, max=num_depth - 1).long()
            
            # Gather confidence at predicted depth
            confidence_bhw = torch.gather(
        ...
        if flip:
            ...
            confidence_bhw = torch.flip(confidence_bhw, (-1,))
        ...
        depth_outputs["confidence_bhw"] = confidence_bhw
    ...
    def compute_losses(self, cur_data, src_data, outputs):
        ...
        # normals_loss = torch.tensor(0.0, dtype=depth_gt.dtype, device=depth_gt.device)

        # loss = ms_loss + sky_loss * 0.1 + 1.0 * grad_loss + 1.0 *  normals_loss
        loss = ms_loss + sky_loss * 0.1 + 1.0 * grad_loss
