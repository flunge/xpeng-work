...
from mvsnet.mvsa.src.mvsanywhere.utils.generic_utils import reverse_imagenet_normalize
...
def quick_viz_export(
    ...
    if valid_mask_b is None:
        device = outputs["depth_pred_s0_b1hw"].device
        shape = outputs["depth_pred_s0_b1hw"].shape
        valid_mask_b = torch.ones((shape[0], 1, shape[-2], shape[-1]), dtype=torch.bool, device=device)
