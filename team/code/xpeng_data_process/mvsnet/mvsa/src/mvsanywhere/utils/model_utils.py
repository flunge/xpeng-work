...
from mvsnet.mvsa.src.mvsanywhere.modules.cost_volume import CostVolumeManager
from mvsnet.mvsa.src.mvsanywhere.modules.feature_volume import FeatureVolumeManager
from mvsnet.mvsa.src.mvsanywhere.modules.view_agnostic_feature_volume import ViewAgnosticFeatureVolumeManager
from mvsnet.mvsa.src.mvsanywhere.experiment_modules.sr_depth_model import DepthModel
...
def load_model_inference(opts, model_class_to_use):
    ...
    if opts.load_weights_from_checkpoint is not None:
        state_dict = torch.load(opts.load_weights_from_checkpoint, weights_only=False)["state_dict"]
        
        # Remove all pix_coords parameters that depend on image resolution
        keys_to_remove = []
        for key in list(state_dict.keys()):
            if 'pix_coords' in key:
                keys_to_remove.append(key)
        
        for key in keys_to_remove:
            state_dict.pop(key, None)
            logger.info(f"Skipped loading {key} due to potential size mismatch")
...
def load_model_training(opts, model_class_to_use):
    ...
    if opts.load_weights_from_checkpoint is not None:
        ...
        model = model_class_to_use(opts)
        state_dict = torch.load(opts.lazy_load_weights_from_checkpoint, weights_only=False)["state_dict"]
