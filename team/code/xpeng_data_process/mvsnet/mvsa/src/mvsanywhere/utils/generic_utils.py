...
def to_gpu(input_dict, key_ignores=None, device=None):
    ...
    for k, v in input_dict.items():
        ...
        if isinstance(v, torch.Tensor):
            input_dict[k] = v.to(device=device, dtype=torch.float32, non_blocking=True)
        elif isinstance(v, list):
            converted_list = []
            for item in v:
                if isinstance(item, torch.Tensor):
                    converted_list.append(item.to(device=device, dtype=torch.float32, non_blocking=True))
                else:
                    converted_list.append(item)
            input_dict[k] = converted_list
        else:
            input_dict[k] = v
    return input_dict
