...
from .utils.general_utils import build_z_supervision_resources
...
from .utils.triangulation import densify_by_triangulation
...
def get_configs():
    ...
    parser.add_argument('--config', default="configs/local_xpeng.yaml", help='config yaml path')
...
def train(configs):
    ...
    if dataset_cfg["dataset"] == "XpengDataset":
        from .datasets.xpeng import XpengDataset as Dataset
    else:
    ...
    road_pointcloud = deepcopy(dataset.road_pointcloud)
    ...
    # 可选：颜色仿射变换模块（本地实现的 AffineTransform + 训练封装）
    affine_module = None
    if affine_cfg is not None:
        # 使用与 reconic 一致的全局 image 索引范围（max_unique_img_idx + 1）
    ...
    z_xy_dist_threshold = opt.get("z_xy_distance_threshold", None)
    if z_xy_dist_threshold is not None:
        z_xy_dist_threshold = float(z_xy_dist_threshold)
        if z_xy_dist_threshold <= 0:
            z_xy_dist_threshold = None
    sample_xyz = None
    z_near_idx = None
    z_near_xy_dist = None
    if road_pointcloud is not None:
        road_xyz = road_pointcloud["xyz"]
        if torch.is_tensor(road_xyz):
            road_xyz = road_xyz.cpu().numpy()
        road_xyz = road_xyz.astype(np.float32)

    gaussian_xy = gaussians.get_xyz[:, :2]
    if OPT_SMOOTH > 0:
        # smooth_near_idx = road.four_indices  # (n,4) faster
        # 使用高斯点自身的KNN(去除自身)构建4近邻索引，确保与当前高斯数量一致
        knn_idx_full = knn_points(gaussian_xy[None], gaussian_xy[None], K=5, return_nn=False).idx.squeeze(0)  # (n,5)
        smooth_near_idx = knn_idx_full[:, 1:5]  # (n,4)
    if OPT_Z:
        sample_xyz, z_near_idx, z_near_xy_dist = build_z_supervision_resources(
            gaussian_xy.detach(), road_xyz, device, z_xy_dist_threshold
        )
        if z_xy_dist_threshold is not None:
            valid_ratio = torch.sum(z_near_xy_dist <= z_xy_dist_threshold)
            print(f"[INFO][RoGS] number of valid points: {valid_ratio}/{len(z_near_xy_dist)} "
                  f"with z_xy_dist_threshold {z_xy_dist_threshold} meters")
    ...
    for epoch in range(opt.epochs):
        # 记录本 epoch 内每个高斯是否曾经对 loss 产生过梯度更新（即在 loss_mask 有贡献）
        ever_in_loss_epoch = torch.zeros(gaussians.get_xyz.shape[0], dtype=torch.bool, device=device)
        # 在最后一个 epoch 开始前执行一次三角化加密（仅一次，不改变后续训练逻辑）
        if epoch == opt.epochs - 1 and OPT_SEG:
            time_start = time.time()
            added = densify_by_triangulation(
                gaussians,
                target_classes=(1, 2, 4),
                edge_samples=1,
                add_triangle_centroid=train_cfg.get("add_triangle_centroid", True),
            )
            if added > 0:
                gaussian_xy = gaussians.get_xyz[:, :2]
                # 为新增的高斯点扩展本 epoch 的参与 loss 统计向量（初值为 False）
                ever_in_loss_epoch = torch.cat(
                    [ever_in_loss_epoch, torch.zeros(added, dtype=torch.bool, device=device)], dim=0
                )
                if OPT_SMOOTH:
                    # 重建平滑用近邻索引，避免形状失配
                    knn_idx_full = knn_points(gaussian_xy[None], gaussian_xy[None], K=5, return_nn=False).idx.squeeze(0)
                    smooth_near_idx = knn_idx_full[:, 1:5]
                if OPT_Z:
                    # 重新计算 z 监督资源，兼容新增高斯
                    sample_xyz, z_near_idx, z_near_xy_dist = build_z_supervision_resources(
                        gaussian_xy, road_xyz, device, z_xy_dist_threshold
                    )
            time_end = time.time()
            print(f"[INFO][RoGS] Triangle densification time: {time_end - time_start} seconds")
            print(f"[INFO][RoGS] Added {added} gaussians. Total gaussians: {gaussian_xy.shape[0]}")
        ...
        for sample in dataloader:
            ...
            if affine_module is not None:
                affine_module.update_lr(iteration)
            ...
            render_image = render_image.permute(1, 2, 0)  # (H, W, 3)

            # 可选：对渲染 RGB 施加仿射颜色变换（按图像索引）
            affine_mat = None
            if affine_module is not None:
                render_image, affine_mat = affine_module.apply(render_image, image_idx)
            ...
            # Affine 仿射正则：鼓励接近单位矩阵与零偏置
            if affine_module is not None:
                total_loss = total_loss + affine_module.regularization_loss(affine_mat)
            ...
            if OPT_Z:
                ...
                if torch.any(surround_filter):
                    ...
                    if z_xy_dist_threshold is not None:
                        cur_near_xy_dist = z_near_xy_dist[surround_filter].squeeze(-1)
                        valid_xy = (cur_near_xy_dist <= z_xy_dist_threshold).float()
                        z_loss = z_loss * valid_xy
            ...
            # 统计本 epoch 内哪些高斯在本 iteration 中实际参与了 loss
            # 优先使用 RGB 的梯度来判断（与颜色更新直接相关），若未使用 RGB 则回退到 z 的梯度
            if gaussians.use_rgb and getattr(gaussians, "_rgb", None) is not None and gaussians._rgb.grad is not None:
                # 对每个高斯点，若任一通道 RGB 梯度非零，则认为本次 iteration 参与了优化
                grad_mask = gaussians._rgb.grad.abs().sum(dim=1) > 0  # (N,)
                ever_in_loss_epoch |= grad_mask
            elif getattr(gaussians, "_z", None) is not None and gaussians._z.grad is not None:
                grad_mask = gaussians._z.grad[:, 0].abs() > 0
                ever_in_loss_epoch |= grad_mask
        ...
        # 在第一个 epoch 结束后，删除在整个 epoch 内从未参与 loss（梯度始终为 0）的高斯点，
        # 并同步更新相关训练资源（平滑邻域、Z 监督等）
        if epoch == 0:
            never_updated_mask = ~ever_in_loss_epoch
            num_pruned_epoch0 = int(never_updated_mask.sum().item())
            if num_pruned_epoch0 > 0:
                logger.info(f"[RoGS][epoch-{epoch}] Pruning {num_pruned_epoch0} gaussians with zero gradients in this epoch.")
                gaussians.prune_points(never_updated_mask)

                # 重新计算与高斯数量相关的缓存（平滑邻域、Z 监督等），避免后续形状不匹配
                gaussian_xy = gaussians.get_xyz[:, :2]
                if OPT_SMOOTH > 0:
                    knn_idx_full = knn_points(gaussian_xy[None], gaussian_xy[None], K=5, return_nn=False).idx.squeeze(0)
                    smooth_near_idx = knn_idx_full[:, 1:5]
    ...
    if train_cfg.save:
        ...
        if affine_module is not None:
            affine_module.save(output_root)
