...
from diff_gaussian_rasterization_depthalpha.scene.cameras import OrthographicCamera
...
class Road(object):
    def __init__(self, config, dataset, device='cuda:0', vis=False):
        ...
        extended_pose_xyz = all_pose_xyz.copy()
        extended_poses = all_poses.copy()
        if config.get("extend_from_last_frame", 0) > 1e-1:
            # get cumsum distance between poses
            extend_length = config.get("extend_from_last_frame", 30)
            cumsum_distance = np.cumsum(np.linalg.norm(all_pose_xyz[1:] - all_pose_xyz[:-1], axis=1))
            last_5_meter_index = np.argmin(abs(cumsum_distance - (cumsum_distance[-1] - 5)))
            last_5_meter_pose = all_pose_xyz[last_5_meter_index]
            
            extend_direction_3d = all_pose_xyz[-1] - last_5_meter_pose
            extend_direction_3d = extend_direction_3d / np.linalg.norm(extend_direction_3d)
            
            # generate extended pose_xyz and pose_matrix in every 5 meters
            extended_length_in_each_interval = np.arange(5, extend_length, 5)
            for length in extended_length_in_each_interval:
                last_pose = all_poses[-1].copy()
                last_pose[:3, 3] += extend_direction_3d * length
                extended_pose_xyz = np.concatenate([extended_pose_xyz, last_pose[:3, 3][None]], axis=0)
                extended_poses = np.concatenate([extended_poses, last_pose[None]], axis=0)

        min_coords = np.min(extended_pose_xyz, axis=0) - self.cut_range
        max_coords = np.max(extended_pose_xyz, axis=0) + self.cut_range
        ...
        # (M, 3)
        vertices, four_indices = cut_point_by_pose(vertices, self.bev_size_pixel, xy_resolution, min_coords, extended_pose_xyz, self.resolution, self.cut_range)
        ...
        # ====> int z and rotation
        traj_point = torch.from_numpy(extended_pose_xyz).float().to(device)
        traj_rotation = torch.from_numpy(extended_poses[:, :3, :3]).float().to(device)
