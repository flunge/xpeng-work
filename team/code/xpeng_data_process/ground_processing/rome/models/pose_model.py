...
class ExtrinsicModel(nn.Module):
    def __init__(self, configs, optim_rotation=False, optim_translation=False, num_camera=None):
        ...
        if optim_rotation:
            self.rotations = nn.Parameter(torch.zeros(size=(num_camera, 3), dtype=torch.float64))  # (N, 3) axis angle
        else:
            rotations = torch.zeros(size=(num_camera, 3), dtype=torch.float64)
            self.register_buffer("rotations", rotations)
        if optim_translation:
            self.translations = nn.Parameter(torch.zeros(size=(num_camera, 3), dtype=torch.float64))  # (N, 3)
        else:
            translations = torch.zeros(size=(num_camera, 3), dtype=torch.float64)
            self.register_buffer("translations", translations)
