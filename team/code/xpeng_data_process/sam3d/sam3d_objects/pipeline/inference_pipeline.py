...
class InferencePipeline:
    def __init__(
        ...
        with self.device:
            ...
            logger.info("\n" + "=" * 70)
            logger.info("MODEL COMPONENTS LOADING TIME REPORT")
            logger.info("=" * 70)
            components_times = [
                ("ss_generator", t2 - t1),
                ("slat_generator", t3 - t2),
                ("ss_decoder", t4 - t3),
                ("ss_encoder", t5 - t4),
                ("slat_decoder_gs", t6 - t5),
                ("ss_condition_embedder", t7 - t6),
                ("slat_condition_embedder", t8 - t7)
            ]
            for component_name, load_time in components_times:
                logger.info(f"  • {component_name:25s}: {load_time:7.3f} seconds")
    ...
    def decode_slat(
        ...
        with torch.no_grad():
            ...
            if "gaussian" in formats:
                ret["gaussian"] = self.models["slat_decoder_gs_4"](slat)
    ...
    def sample_sparse_structure_multi_view(
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=self.shape_model_dtype):
                ...
                max_coords = 5500
                final_downsample_factor = 1.0
                downsample_factor = 1.2
                while coords.shape[0] > max_coords:
                    print("coords.shape[0] ", coords.shape[0])
                    coords = downsample_sparse_structure(coords, max_coords, downsample_factor)
                    final_downsample_factor *= downsample_factor

                # logger.info(
                #     f"[Stage 1 Multi-view] Downsampled coords from {original_shape[0]} to {coords.shape[0]}"
                # )
                return_dict["coords"] = coords
                return_dict["downsample_factor"] = final_downsample_factor
