...
class Dino(torch.nn.Module):
    def __init__(
        self,
        input_size: int = 224,
        repo_or_dir: str = "/workspace/group_share/adc-sim/users/wangyd13/facebookresearch_dinov2_main",
        ...
        with warnings.catch_warnings():
            ...
            backbone_kwargs["pretrained"] = False # 禁用自动下载
            ...
            local_weights_path="/workspace/group_share/adc-sim/users/wangyd13/checkpoints/dinov2_vitl14_reg4_pretrain.pth"
            ...
            if local_weights_path and os.path.exists(local_weights_path):
                ...
                # logger.info(f"Successfully loaded local weights for {dino_model}")
            elif local_weights_path:
                raise FileNotFoundError(f"Local weights file not found: {local_weights_path}")
            else:
                logger.warning("No local weights path provided, model will use random weights!")
