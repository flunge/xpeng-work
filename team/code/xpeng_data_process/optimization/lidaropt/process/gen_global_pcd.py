...
def gen_data_for_lidar2cam(data_root=None, seg_dir=None, cam=None, num_pcd_cvt=50, apply_delta_lidar2ego=False):
    """
    Args:
        data_root: 数据根目录
        seg_dir: segmentation目录
        cam: 相机名称
        num_pcd_cvt: 点云转换数量
        apply_delta_lidar2ego: 是否应用delta_lidar2ego变换
    """
    if data_root is None:
        ...
        cam = args.cam
        num_pcd_cvt = int(args.num_pcd_cvt) if args.num_pcd_cvt else 50
        apply_delta_lidar2ego = args.apply_delta_lidar2ego

    cam2_truncated_height = 792 # 大于cam2_truncated_height 为ego_mask区域

    # ------------------------------------------------------------------------------------
    # 读取lidar外参, 相机内参外参, cam_to_worlds
    transform_json = json.load(open(os.path.join(data_root, "transform_ego_fix.json")))
    ...
    for t in tqdm(range(len(filenames))):
        ...
        road_mask = cv2.imread(seg_dir+'/'+cam+'/'+fn+'.png', cv2.IMREAD_ANYDEPTH)
        ...
        dyn_mask = np.isin(road_mask, dyn_set).astype(np.uint8) * 255
        ...
        for u in range(len(depth)):
            if (cam_points[u, 0] >= 0)& (cam_points[u, 0] <= (width-1) )& (cam_points[u, 1] >= 0)& (cam_points[u, 1] <= truncated_height): # (1080-1) 792
                if (depth[u] > 0) & (depth[u] < 150):
                    ...
                    if road_mask[round( cam_points[u, 1]), round( cam_points[u, 0])] in grd_set:
