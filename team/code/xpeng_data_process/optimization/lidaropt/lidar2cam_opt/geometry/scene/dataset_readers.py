...
def readKittiCamera2(uid, pc, pose, name, pc_path, data_path):
    file_name = data_path + "/" + "00.png"
    img = cv2.imread(file_name)
    width=img.shape[1]
    height=img.shape[0]

    # cam0
    intr = np.loadtxt(data_path + "/" + "intr.txt")
    e2c = np.loadtxt(data_path + "/" + "ego2cam.txt")
    # print(intr)
    # print(e2c)
    ...
    for i in range(N):
        # 6个旋转视角覆盖45度fov
        r1 = np.eye(4)
        r1[:3,:3] = Rotation.from_euler("xyz", [0, 0, (1/4 * math.pi / N * i) - (1/8 * math.pi)]).as_matrix()
        ext = r0 @ r1
        
        pc_camera = ext[:3, :3] @ pc[:, :3].transpose() + ext[:3, 3].reshape(3,1)
        z = pc_camera[2]
        zsort = z.argsort()[::-1]
        z = z[zsort]
        refl = pc[:, 3][zsort]
...
def readScanOfKittiPinhole(data_path, pose_path, num, start_frame):
    ...
    for idx in range(num):
        ...
        pc = pc[(r < 100 * 100)]
        pcs.append(pc)
        cam_infos += readKittiCamera2(idx, pc, poses[idx], f"{start_frame+idx:06}", pc_path, data_path) # readKittiCamera
