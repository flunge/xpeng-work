    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if (iteration > opt.iterations - 5):
                ...
                gt_depth[(uv[:, 1]*dh*0.5+dh*0.5).long(), (uv[:, 0]*dw*0.5+dw*0.5).long()] = gt_image.float()
                # gt_depth[(uv[:, 1]).long(), (uv[:, 0]).long()] = gt_image.float()
...
def run_2dgs(data_path, output_dir, save_dir, **kwargs):
    """
    Args:
        data_path: 数据路径，对应option.yml中的data.path
        output_dir: 输出目录，对应option.yml中的output
        save_dir: 保存目录，对应--save_dir参数，传入ModelParams
        **kwargs: 其他可选参数
    """
    config = {
        "data": {
    ...
    test_iterations = kwargs.get("test_iterations", [7_000, 30_000])
    save_iterations = kwargs.get("save_iterations", [7_000, 30_000])
    checkpoint_iterations = kwargs.get("checkpoint_iterations", [15_000, 30_000])
    start_checkpoint = kwargs.get("start_checkpoint", None)
    detect_anomaly = kwargs.get("detect_anomaly", False)
    ...
    class Args:
        def __init__(self):
            ...
            self.quiet = quiet
            self.iterations = kwargs.get("iterations", 30_000)
    ...
    # print("Optimizing " + str(opt.get("model_path", "unknown")))
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description="Training script parameters")
    parser.add_argument('--config', type=str, default="option.yml", help="Path to config file")
