...
class KITTILoader(object):
    ...
    def __init__(self, config={}):
        ...
        file_name = self.config["root_path"] + "/" + "00.png"
        img = cv2.imread(file_name)
        width=img.shape[1]
        height=img.shape[0]

        intr = np.loadtxt(self.config["root_path"] + "/" + "intr.txt")
        print(intr)
        self.cam = PinholeCamera(width, height, intr[0,0], intr[1,1], intr[0,2], intr[1,2])
    ...
    def __next__(self):
        if self.img_id < self.img_N:
            # file_name = self.config["root_path"] + "/sequences/" + self.config["sequence"] \
            #             + "/image_0/" + str(self.img_id).zfill(2) + ".png" # .zfill(6)
            
            file_name = self.config["root_path"] + "/" + str(self.img_id).zfill(2) + ".png" # .zfill(6)
