...
def create_pose(opt, gt_ext):
    ...
    R_params = {"params": ext.R.parameters(), "lr": 1e-7} # 1e-2 1e-7
...
def train_gaussian(opt, train_cams, feature_db, gt_ext, writer):
    ...
    for it in progress:
        ...
        if it > 1000 and flagR==float("inf")  and changed[1] < 0.1:
            optimizer_pose.param_groups[0]["lr"] = 1e-7 # 1e-3 1e-7
...
def parse_arg(arg, opt):
    ...
    if opt_agrs.config != "option.yml":
        new_opt = parse(opt_agrs.config)
        opt.update(new_opt)

    opt.update(vars(opt_agrs))
    scene, frame, name = opt.g.split("-")
    # opt.data.scene = int(scene)
    opt.data.scene = scene
    opt.data.start_frame = int(frame)
    opt.data.num = 50
    opt.data.name = name

def run_calibration(data_path, output_dir, save_dir, render=True, **kwargs):
    """
    Args:
        data_path: 数据路径，对应option.yml中的data.path
        output_dir: 输出目录，对应option.yml中的output
        save_dir: 保存目录，对应-g参数
        render: 是否渲染，对应--render参数
        **kwargs: 其他可选参数
    """
    try:
        config = {
        ...
        scene, frame, name = save_dir.split("-")
        opt.data.scene = scene
        opt.data.start_frame = int(frame)
        opt.data.num = 50
        opt.data.name = name
