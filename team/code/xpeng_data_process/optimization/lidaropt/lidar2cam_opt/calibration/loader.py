...
class Loader():
    ...
    def load_cams(opt):
        ...
        calib_path = os.path.join(opt.data.path, "calib.txt")
        intr, extr, ext0 = Loader.load_calib2(calib_path) # load_calib
        # load poses
        poses = Loader.load_poses2(opt, ext0) # load_poses
    ...
    @staticmethod
    def load_poses2(opt, ext0):
        # pose_path = os.path.join(opt.data.path, "poses", f"{opt.data.scene:02}.txt")
        pose_path = os.path.join(opt.data.path, "egoposes.txt")
        poses_np = np.loadtxt(pose_path)[opt.data.start_frame:opt.data.num+opt.data.start_frame]
        poses = SE3(mat=torch.from_numpy(poses_np).float().reshape(-1, 3, 4).cuda())
        poses = poses[0].invert() @ poses
        return poses.invert()
    ...
    @staticmethod
    def load_calib2(calib_file):
        # calib_file = os.path.join(basic_path, "calib.txt")
        trinsics = torch.from_numpy(np.loadtxt(calib_file, usecols=range(1,13)).reshape(-1, 3, 4)).float()
        ext0 = SE3(mat=trinsics[1].cuda())
        intr = SE3(R=trinsics[0,:,:3].cuda())
