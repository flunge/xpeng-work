...
def load_mask_from_memory(buffer, to_tensor=True):
...
class MaskDictionaryModel:
    ...
    def update_masks(self, tracking_annotation_dict, iou_threshold=0.8, objects_count=0):
        ...
        for seg_obj_id, seg_mask in self.labels.items():  # 检测结果
            flag = 0 
            new_mask_copy = ObjectInfo()
            if seg_mask.mask.sum() == 0:
                continue
            
            for object_id, object_info in tracking_annotation_dict.labels.items():  # grounded_sam masks     以前的跟踪结果
        ...
        for object_id, object_info in tracking_annotation_dict.labels.items():
            if object_id not in updated_masks:
...
class ObjectInfo:
    ...
    def get_box(self):
        input_boxes = np.array([self.x1, self.y1, self.x2, self.y2])
        return input_boxes
    ...
    def to_dict(self):
        return {
            ...
            "y2": self.y2,
            "logit": self.logit,
            "mask": save_mask_to_memory(self.mask),
