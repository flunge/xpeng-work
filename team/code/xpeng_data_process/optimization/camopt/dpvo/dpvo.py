...
class DPVO:
    ...
    def __init__(self, cfg, network, ht=480, wd=640, viz=False):
        ...
        self.se3_identity = SE3.Identity(1, device="cuda")
    ...
    def __call__(self, tstamp, image, intrinsics, mask):
        """ track new frame """
        ...
        if self.n > 0 and not self.is_initialized:
            if self.motion_probe() < 2.0:
                self.pg.delta[self.counter - 1] = (self.counter - 2, self.se3_identity[0])
                return
