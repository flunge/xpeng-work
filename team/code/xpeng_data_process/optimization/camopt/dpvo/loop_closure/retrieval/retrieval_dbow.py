...
class RetrievalDBOW:
    ...
    def _detect_loop(self, thresh, num_repeat=1):
        ...
        while True:
            try:
                i, (score, j, _) = self.out_queue.get(timeout=120)
                break
            except QueueEmpty:
                if not self.proc.is_alive():
                    raise RuntimeError(
                        f"[RetrievalDBOW][ERROR] DBoW subprocess (pid={self.proc.pid}) died "
                        f"with exitcode={self.proc.exitcode}. Cannot retrieve loop closure."
                    )
                continue
        self.being_processed -= 1
        if score < thresh:
