...
class LongTermLoopClosure:
    ...
    def lc_callback(self, skip_if_empty=True):
        ...
        self.lc_in_progress = False
        try:
            final_est = self.result_queue.get(timeout=120)
        except Exception:
            # Check if async PGO process is still running
            if self.lc_process.ready():
                print(f"[LongTermLoopClosure][ERROR] PGO process finished but result_queue is empty. "
                      f"PGO may have failed silently.")
                return
            print(f"[LongTermLoopClosure][WARN] result_queue.get() timed out, PGO may still be running.")
            return
