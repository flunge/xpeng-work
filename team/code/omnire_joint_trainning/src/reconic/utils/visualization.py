...
def get_layout(dataset_type: str):
    # TODO(ryan): Remove this function.
    # layout functions should be defined in thier respective dataset classes.
...
def layout_xpeng(imgs: List[np.array], cam_names: List[str]) -> np.array:
    """Combine cameras into a tiled image.
    Layout:
    ...
    for idx, cam_name in enumerate(cam_names):
        img = imgs[idx]
        downsampled_img = cv2.resize(img, (960, 720), interpolation=cv2.INTER_CUBIC)  # (720, 960, 3)
        if cam_name == "cam0":
            tiled_img[:landscape_height, :landscape_width] = downsampled_img
            filled_mask[:landscape_height, :landscape_width] = 1
        elif cam_name == "cam7":
            tiled_img[:landscape_height, landscape_width:] = downsampled_img
            filled_mask[:landscape_height, landscape_width:] = 1
        elif cam_name == "cam3":
            tiled_img[landscape_height : 2 * landscape_height, :landscape_width] = downsampled_img
            filled_mask[landscape_height : 2 * landscape_height, :landscape_width] = 1
        elif cam_name == "cam4":
            tiled_img[landscape_height : 2 * landscape_height, landscape_width:] = downsampled_img
            filled_mask[landscape_height : 2 * landscape_height, landscape_width:] = 1
        elif cam_name == "cam2":
            tiled_img[2 * landscape_height : 3 * landscape_height, :landscape_width] = downsampled_img
            filled_mask[2 * landscape_height : 3 * landscape_height, :landscape_width] = 1
        elif cam_name == "cam5":
            tiled_img[3 * landscape_height :, :landscape_width] = downsampled_img
            filled_mask[3 * landscape_height :, :landscape_width] = 1
        elif cam_name == "cam6":
            tiled_img[3 * landscape_height :, landscape_width:] = downsampled_img
            filled_mask[3 * landscape_height :, landscape_width:] = 1
...
depth_visualizer = partial(visualize_depth, lo=4.0, hi=120, depth_curve_fn=_np_log_eps)
