...
def safe_binary_cross_entropy(
    input: torch.Tensor, target: torch.Tensor, weight: torch.Tensor = None, limit: float = 0.1, reduction="mean"
) -> torch.Tensor:
...
def binary_cross_entropy(
    input: torch.Tensor, target: torch.Tensor, weight: torch.Tensor = None, reduction="mean"
) -> torch.Tensor:
    loss = F.binary_cross_entropy(input, target, weight=weight, reduction="none")
    return reduce(loss, None, reduction=reduction)
...
class DepthLoss(nn.Module):
    ...
    def __call__(
        ...
        if self.depth_error_percentile is not None:
            ...
            depth_error = depth_error[depth_error.argsort()[: int(len(depth_error) * self.depth_error_percentile) + 1]]
