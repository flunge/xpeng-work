...
from ..gaussians.basics import RGB2SH, dataclass_camera, interpolate_quats, num_sh_bases, quat_mult
from ..gaussians.vanilla import VanillaGaussians
from ..human_body import (
    SMPLTemplate,
...
class SMPLNodes(RigidNodes):
    ...
    def transform_means(self, means: torch.Tensor) -> torch.Tensor:
        ...
        W, A = self.template(
            ...
            xyz_canonical=(
                means.reshape(self.num_instances, self.smpl_points_num, 3) if self.use_voxel_deformer else None
            ),
        )
        T = torch.einsum("bnj, bjrc -> bnrc", W, A)
        R = T[:, :, :3, :3]  # [N, 3, 3]
    ...
    def deform_gaussian_points(
        ...
        W, A = self.template(
            ...
            instances_mask=instance_mask,
            xyz_canonical=(
                means.reshape(1, self.smpl_points_num, 3).repeat(self.num_instances, 1, 1)
                if self.use_voxel_deformer
                else None
            ),
        )
    ...
    def replace_instances(self, replace_dict: Dict[int, int]) -> None:
        ...
        for ins_id, new_id in replace_dict.items():
            ...
            # self.instances_quats = Parameter(torch.cat([self.instances_quats,
            #                                             new_gaussian["instances_quats"].unsqueeze(1)], dim=1))
