...
class WaymoProcessor(object):
    ...
    def convert_one(self, file_idx):
        ...
        for frame_idx, data in enumerate(tqdm(dataset, desc=f"File {file_idx}", total=num_frames, dynamic_ncols=True)):
            ...
            if (
                ...
                not in self.selected_waymo_locations  # pylint: disable=unsupported-membership-test
            ):
                continue
    ...
    def save_interested_labels(self, frame, file_idx):
        ...
        object_counts = {label_pb2.Label.Type.Name(x.type): x.count for x in frame.context.stats.camera_object_counts}
    ...
    def save_objects(self, dataset):
        ...
        for frame_idx, data in enumerate(dataset):
            ...
            for laser_label in frame.laser_labels:
                ...
                str_id = str(laser_label.id)
                if WAYMO_CLASSES[laser_label.type] not in WAYMO_DYNAMIC_CLASSES:
                    continue
                ...
                if str_id not in instances_info:
                    instances_info[str_id] = dict(
                        id=laser_label.id,
                        # class_ind=l.type,
                        class_name=WAYMO_CLASSES[laser_label.type],
                        frame_annotations={
                            "frame_idx": [],
                            "obj_to_world": [],
                            "box_size": [],
