...
class WaymoLiDARSource(SceneLidarSource):
    ...
    def load_lidar(self):
        ...
        for t in trange(0, len(self.lidar_filepaths), desc="Loading lidar", dynamic_ncols=True):
            ...
            lidar_origins = torch.from_numpy(lidar_info[:, :3].copy()).float()
            lidar_points = torch.from_numpy(lidar_info[:, 3:6].copy()).float()
            lidar_ids = torch.from_numpy(lidar_info[:, 13].copy()).float()
            lidar_flows = torch.from_numpy(lidar_info[:, 6:9].copy()).float()
            lidar_flow_classes = torch.from_numpy(lidar_info[:, 9].copy()).long()
            ground_labels = torch.from_numpy(lidar_info[:, 10].copy()).long()
