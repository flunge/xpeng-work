...
from ..tools.multiprocess_utils import track_parallel_progress
...
class NuScenesProcessor(object):
    ...
    def __init__(
        ...
        assert self.interpolate_N <= 4, "Interpolation frames should be less than 4. or there will be frame drop issue."
    ...
    def save_calib(self, scene_data, scene_idx):
        ...
        while True:
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                np.savetxt(
                    f"{self.save_dir}/{str(scene_idx).zfill(3)}/intrinsics/" f"{str(cam_idx)}.txt",
                    Ks,
                )
    ...
    def save_lidar(self, scene_data, scene_idx):
        ...
        while True:
            ...
            np.savetxt(
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/lidar_pose/" f"{str(key_frame_idx).zfill(3)}.txt",
                lidar_to_world,
            )
    ...
    def save_lidar_interpolated(self, scene_data, scene_idx, timestamps: np.array):
        ...
        for frame_idx, token in enumerate(closest_tokens):
            ...
            # Save lidar pose in world coordinates
            np.savetxt(
                f"{self.save_dir}/{str(scene_idx).zfill(3)}/lidar_pose/" f"{str(frame_idx).zfill(3)}.txt",
                lidar_to_world,
            )
    ...
    def save_dynamic_mask(self, scene_data, scene_idx, class_valid="all"):
        ...
        while True:
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                dynamic_mask_path = (
                    f"{self.save_dir}/{str(scene_idx).zfill(3)}/dynamic_masks"
                    f"/{class_valid}/{str(key_frame_idx).zfill(3)}_{str(cam_idx)}.png"
                )
    ...
    def save_dynamic_mask_interpolated(self, scene_data, scene_idx, timestamps, instances_info, class_valid="all"):
        ...
        for frame_idx, timestamp in enumerate(timestamps):
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                dynamic_mask = Image.fromarray(dynamic_mask, "L")
                dynamic_mask_path = (
                    f"{self.save_dir}/{str(scene_idx).zfill(3)}/"
                    f"dynamic_masks/{class_valid}/{str(frame_idx).zfill(3)}_{str(cam_idx)}.png"
                )
                dynamic_mask.save(dynamic_mask_path)
