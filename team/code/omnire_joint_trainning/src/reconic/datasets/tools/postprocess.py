...
def fill_discrete_features(mask, features):
    """Fill missing discrete labels by nearest valid neighbor (no interpolation)."""
    features = features.clone()
    n = features.shape[0]
    mask_len = mask.shape[0]
    # 先做前向填充
    ...
    for i in range(n):
        if i >= mask_len:
            continue
        if mask[i]:
            last_valid = features[i]
        elif last_valid is not None:
            features[i] = last_valid
    ...
    for i in range(n - 1, -1, -1):
        ...
        if mask[i]:
            ...
            features[i] = last_valid
        # 如果全缺失，这里可能仍为原始值，但你的场景应有至少一个有效帧
    return features
...
def match_and_postprocess(
    ...
    collector = {}
    num_c_padding = num_c + 1
    # Initialize the collector
    for gt_tid in valid_gt_tids:
        collector[gt_tid] = {
            "smpl": {
                "global_orient": torch.zeros(num_c_padding, num_f, 1, 3, 3),
                "body_pose": torch.zeros(num_c_padding, num_f, 23, 3, 3),
                "betas": torch.zeros(num_c_padding, num_f, 10),
            },
            "camera": torch.zeros(num_c_padding, num_f, 3),
            "2DBox_appear_mask": torch.zeros(num_c_padding, num_f).bool(),
            "area": torch.zeros(num_c_padding, num_f),
            "matched_mask": torch.zeros(num_c_padding, num_f).bool(),
        }
    ...
    for gt_tid in valid_gt_tids:
        ...
        print(f"best_cam_id: {best_cam_id}, masked_best_cam_id: {masked_best_cam_id}")
    ...
    for gt_tid in valid_gt_tids:
        ...
        if need_fill.any():
            ...
            print(f'shape check, mask shape: {mask.shape}, feature_masked shape: {feature_masked.shape}, mask: {mask}')
            feature_full = interpolate_features(mask, feature_masked)
            ...
            feature_full = fill_discrete_features(mask, feature_masked)
            print(f"3. Interpolated selected_cam_idx: {feature_full}")
    ...
    for gt_tid in valid_gt_tids:
        ...
        if need_fill.any():
            ...
            feature_masked = merged_collector[gt_tid]["selected_cam_idx"][final_complete_mask]
            feature_full = fill_discrete_features(mask, feature_masked)
            print(f"4. Interpolated selected_cam_idx: {feature_full}")
            merged_collector[gt_tid]["selected_cam_idx"][final_complete_mask] = feature_full.round()
