...
def run_4DHumans(
    ...
    for cam_id in camera_list:
        ...
        if not os.path.exists(cached_video_path):
            ...
            image_paths = sorted(glob(os.path.join(images_dir, f"cam{cam_id}", "*.*")))
    ...
    for i, cam_id in enumerate(camera_list):
        if already_done:
            ...
            pred_tracks_allcam[cam_id] = joblib.load(os.path.join(temp_dir, "phalp_output", "results", f"demo_{cam_id}.pkl"))
    ...
    phalp_output_results_dir = os.path.join(temp_dir, "phalp_output", "results")
    if os.path.exists(phalp_output_results_dir):
        shutil.rmtree(os.path.join(temp_dir, "phalp_output", "results"))
