...
class GaussinaModelSky(GaussianModel):
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):    
        ...
        pointcloud_rotations = np.zeros((pointcloud_xyz.shape[0], 4))
        pointcloud_rotations[:, 0] = 1 

        scales = np.zeros((pointcloud_xyz.shape[0], 3))
        opacities = np.zeros((pointcloud_xyz.shape[0], 1))

        pcd = BasicPointCloud(pointcloud_xyz, pointcloud_rgb, pointcloud_normal, pointcloud_rotations, scales, opacities)
