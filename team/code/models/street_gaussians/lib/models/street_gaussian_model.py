...
from lib.models.gaussian_model_grd import GaussianModelGround
...
from lib.models.color_correction import ColorCorrection, ColorCorrectionPixelAware
...
class StreetGaussianModel(nn.Module):
    def __init__(self, metadata):
        ...
        self.use_tracklets_world = cfg.model.gaussian.use_tracklets_world

        # background + moving objects
        self.include_background = cfg.model.nsg.get('include_bkgd', True)
        self.include_ground = cfg.model.nsg.get('include_ground', True)
        self.include_obj = cfg.model.nsg.get('include_obj', True)
        self.include_quasi_static_obj = cfg.model.nsg.get('include_quasi_static_obj', False)
    ...
    def get_visibility(self, model_name):
        if model_name == 'background':
            if model_name in self.include_list and self.include_background:
                ...
                return False
        elif model_name == 'ground':
            if model_name in self.include_list and self.include_ground:
                return True
            else:
                return False
        elif model_name == 'sky':
            ...
            if model_name in self.include_list and (self.include_obj or self.include_quasi_static_obj):
                return True
            else:
                return False
        else:
    ...
    def create_from_pcd(self, pcd_dict: dict, spatial_lr_scale: float):
        for model_name in self.model_name_id.keys():
            model: GaussianModel = getattr(self, model_name)
            if model_name in ['background', 'sky', 'ground']:
                model.create_from_pcd(pcd_dict[model_name], spatial_lr_scale)
            else:
    ...
    def save_ply_vis(self, dir_path):
        mkdir_p(dir_path)
        
        for i in range(self.models_num):
            model_name = self.model_name_id.inverse[i]
            model: GaussianModel = getattr(self, model_name)
            plydata = model.make_ply_visualization()
            plydata = PlyElement.describe(plydata, 'vertex')
            plydata_list = [plydata]
            sub_path = os.path.join(dir_path, f'vis_{model_name}.ply')
            PlyData(plydata_list).write(sub_path)

            plydata = model.make_ply()
            plydata = PlyElement.describe(plydata, 'vertex')
            plydata_list = [plydata]
            sub_path = os.path.join(dir_path, f'model_{model_name}.ply')
            PlyData(plydata_list).write(sub_path)

    def save_ply_model(self, dir_path):
        mkdir_p(dir_path)
        for i in range(self.models_num):
            model_name = self.model_name_id.inverse[i]
            model: GaussianModel = getattr(self, model_name)
    ...
    def load_state_dict(self, state_dict, exclude_list=[]):
        for model_name in self.model_name_id.keys():
            ...
            try:
                ...
                print(f'[ERROR] Failed to load state_dict of model {model_name}: {e}')
        
        if self.actor_pose is not None:
            try:
                self.actor_pose.load_state_dict(state_dict['actor_pose'])
            except Exception as e:
                print(f'[ERROR] Failed to load state_dict of actor pose: {e}')
        ...
        if self.color_correction is not None:
            try:
                ...
                print(f'[ERROR] Failed to load state_dict of color correction: {e}. Train from scratch.')
    ...
    def setup_functions(self):
        ...
        obj_tracklets_world = self.metadata['obj_tracklets_world']
        ...
        # Build ground model separately
        if self.include_ground:
            self.ground = GaussianModelGround(
                model_name='ground', 
                scene_center=self.metadata['scene_center'],
                scene_radius=self.metadata['scene_radius'],
                sphere_center=self.metadata['sphere_center'],
            ...
            self.model_name_id['ground'] = self.models_num
            self.models_num += 1

        # Build object model
        for track_id, obj_meta in self.obj_info.items():
            if self.include_obj or (self.include_quasi_static_obj and obj_meta["keep_in_static_scene"]) == True:
                model_name = f'obj_{track_id:09d}'
                print("keep_in_static_scene model_name:", model_name)
        ...
        # Build actor model 
        if self.include_obj or self.include_quasi_static_obj:
            self.actor_pose = ActorPose(obj_tracklets, 
                obj_tracklets_world, tracklet_timestamps, camera_timestamps, obj_info)
        else:
    ...
    def parse_camera(self, camera: Camera, is_render=False):
        # set camera
        ...
        # ground
        if self.get_visibility('ground'):
            num_gaussians_ground = self.ground.get_xyz.shape[0]
            self.num_gaussians += num_gaussians_ground
        ...
        if self.get_visibility('ground'):
            num_gaussians_ground = self.ground.get_xyz.shape[0]
            self.graph_gaussian_range['ground'] = [idx, idx+num_gaussians_ground-1]
            idx += num_gaussians_ground
        ...
        if len(self.graph_obj_list) > 0:
            self.obj_rots = []
            self.obj_trans = []
            if not self.use_tracklets_world:
                for i, obj_name in enumerate(self.graph_obj_list):
                    ...
                    ego_pose = self.viewpoint_camera.ego_pose_smoothed
                    ego_pose_rot = self.viewpoint_camera.ego_pose_smoothed_rot
                ...
                for i, obj_name in enumerate(self.graph_obj_list):
                    ...
                    idx, _ = self.actor_pose.find_closest_indices(track_id, timestamp)
                    frame_ind, column_ind = idx[0], idx[1]
                    if self.actor_pose.opt_track:
                        trans = self.actor_pose.input_trans_world[frame_ind, column_ind] + self.actor_pose.opt_trans[frame_ind, column_ind]
                        rot = self.actor_pose.input_rots_world[frame_ind, column_ind] + self.actor_pose.opt_rots[frame_ind, column_ind]
                    else:
                        trans = self.actor_pose.input_trans_world[frame_ind, column_ind]
                        rot = self.actor_pose.input_rots_world[frame_ind, column_ind]
                    obj_rot = rot.expand(obj_model.get_xyz.shape[0], -1)
                    obj_trans = trans.unsqueeze(0).expand(obj_model.get_xyz.shape[0], -1)
                    self.obj_rots.append(obj_rot)
                    self.obj_trans.append(obj_trans)
            ...
            for obj_name in self.graph_obj_list:
                obj_model: GaussianModelActor = getattr(self, obj_name)
                if is_render or obj_model.deformable or self.flip_prob == 0:
                    flip_mask = torch.zeros_like(obj_model.get_xyz[:, 0]).bool().cuda()
                else:
                    # flip_mask = torch.rand_like(obj_model.get_xyz[:, 0]) < self.flip_prob
                    flip_mask = torch.arange(obj_model.get_xyz.shape[0]) % 2 == 0
                    flip_mask = flip_mask.bool().cuda()
                self.flip_mask.append(flip_mask)
            self.flip_mask = torch.cat(self.flip_mask, dim=0)   

    def parse_camera_dynamic_objects(self, camera: Camera, is_render=False):
        # set camera
        print(f"StreetGaussianModel::parse_camera_dynamic_objects", flush=True)
        self.viewpoint_camera = camera
        ...
        if len(self.graph_obj_list) > 0:
            ...
            self.obj_trans = []
            if not self.use_tracklets_world:
                print(f"StreetGaussianModel::parse_camera not use_tracklets_world: {len(self.graph_obj_list)}", flush=True)
                for i, obj_name in enumerate(self.graph_obj_list):
                    obj_model: GaussianModelActor = getattr(self, obj_name)
                    track_id = obj_model.track_id
                    print(f"StreetGaussianModel::parse_camera track_id: {track_id}", flush=True)
                    if track_id != 999:
                        print(f"parse_camera skip: {track_id}", flush=True)
                        continue
                    obj_rot = self.actor_pose.get_tracking_rotation(track_id, self.viewpoint_camera)
                    obj_trans = self.actor_pose.get_tracking_translation(track_id, self.viewpoint_camera)                
                    ego_pose = self.viewpoint_camera.ego_pose
                    ego_pose_rot = self.viewpoint_camera.ego_pose_rot
                    obj_rot = quaternion_raw_multiply(ego_pose_rot.unsqueeze(0), obj_rot.unsqueeze(0)).squeeze(0)
                    obj_trans = ego_pose[:3, :3] @ obj_trans + ego_pose[:3, 3]
                ...
                print(f"StreetGaussianModel::parse_camera use_tracklets_world: {len(self.graph_obj_list)}", flush=True)
                for i, obj_name in enumerate(self.graph_obj_list):
                    obj_model: GaussianModelActor = getattr(self, obj_name)
                    track_id = obj_model.track_id
                    print(f"StreetGaussianModel::parse_camera 2 track_id: {track_id}", flush=True)
                    if track_id != 999:
                        print(f"parse_camera skip: {track_id}", flush=True)
                        continue
                    timestamp = self.viewpoint_camera.meta['timestamp']
    ...
    def get_scaling(self):
        ...
        if self.get_visibility('ground'):
            scaling_ground = self.ground.get_scaling
            scalings.append(scaling_ground)
    ...
    def get_rotation(self):
        ...
        if self.get_visibility('ground'):
            rotations_ground = self.ground.get_rotation
            if self.use_pose_correction:
                rotations_ground = self.pose_correction.correct_gaussian_rotation(self.viewpoint_camera, rotations_ground)
            rotations.append(rotations_ground)
    ...
    def get_xyz(self):
        ...
        if self.get_visibility('ground'):
            xyz_ground = self.ground.get_xyz
            if self.use_pose_correction:
                xyz_ground = self.pose_correction.correct_gaussian_xyz(self.viewpoint_camera, xyz_ground)
            xyzs.append(xyz_ground)
    ...
    def get_features(self):                
        ...
        if self.get_visibility('ground'):
            features_ground = self.ground.get_features
            features.append(features_ground)
        
        for i, obj_name in enumerate(self.graph_obj_list):
            obj_model: GaussianModelActor = getattr(self, obj_name)
            feature_obj = obj_model.get_features_fourier(self.frame, obj_name=obj_name)
    ...
    def get_sh_python_color(self, camera_center):
        for i, obj_name in enumerate(self.graph_obj_list):
            obj_model: GaussianModelActor = getattr(self, obj_name)
            feature_obj = obj_model.get_features_fourier(self.frame, obj_name=obj_name)
            shs_view = feature_obj.transpose(1, 2).view(-1, 3, (obj_model.max_sh_degree+1)**2)
            dir_pp = (obj_model.get_xyz - camera_center.repeat(feature_obj.shape[0], 1))
            dir_pp_normalized = dir_pp / dir_pp.norm(dim=1, keepdim=True)
            sh2rgb = eval_sh(obj_model.active_sh_degree, shs_view, dir_pp_normalized)
            colors_precomp = torch.clamp_min(sh2rgb + 0.5, 0.0)
            # if obj_name == "obj_000000999":
            #     obj_model.color_list[self.frame] = {
            #         "min_color": colors_precomp.min().item(),
            #         "max_color": colors_precomp.max().item()
            #     }
    ...
    def get_colors(self, camera_center):
        ...
        if self.get_visibility('ground'):
            model_names.append('ground')

        model_names.extend(self.graph_obj_list)

        for model_name in model_names:
            if model_name in ['background', 'ground']:                
                model: GaussianModel= getattr(self, model_name)
            else:
    ...
    def get_semantic(self):
        ...
        if self.get_visibility('ground'):
            semantic_ground = self.ground.get_semantic
            semantics.append(semantic_ground)
    ...
    def get_opacity(self):
        ...
        if self.get_visibility('ground'):
            opacity_ground = self.ground.get_opacity
            opacities.append(opacity_ground)
    ...
    def get_normals(self, camera: Camera):
        ...
        if self.get_visibility('ground'):
            normals_ground = self.ground.get_normals(camera)
            normals.append(normals_ground)
    ...
    def set_max_radii2D(self, radii, visibility_filter):
        ...
        for model_name in self.graph_gaussian_range.keys():
            ...
            if len(radii.shape) == 1:
                ...
                max_radii2D_model = torch.max(radii[start:end], dim=1)[0]
    ...
    def add_densification_stats(self, *args, **kwargs):
        if cfg.train.get('use_gsplat', False):
            self.add_densification_stats_gsplat(*args, **kwargs)
        else:
            self.add_densification_stats_diff_gaus_raster(*args, **kwargs)

    def add_densification_stats_diff_gaus_raster(self, viewspace_point_tensor, visibility_filter, cam_width, cam_height):
    ...
    def add_densification_stats_gsplat(self, viewspace_point_tensor, visibility_filter, cam_width, cam_height):
        viewspace_point_tensor_absgrad = viewspace_point_tensor.absgrad[0]
        viewspace_point_tensor_grad = viewspace_point_tensor.grad[0]
        ...
        for model_name in self.graph_gaussian_range.keys():
            ...
            visibility_model = visibility_filter[start:end]
            grad_model = viewspace_point_tensor_grad[start:end]
            absgrad_model = viewspace_point_tensor_absgrad[start:end]
            model.xyz_gradient_accum[visibility_model, 0] += torch.sqrt(
                (grad_model[visibility_model, 0] * cam_width / 2.0) ** 2 +
                (grad_model[visibility_model, 1] * cam_height / 2.0) ** 2)
            model.xyz_gradient_accum[visibility_model, 1] += torch.sqrt(
                (absgrad_model[visibility_model, 0] * cam_width / 2.0) ** 2 +
                (absgrad_model[visibility_model, 1] * cam_height / 2.0) ** 2)
            model.denom[visibility_model] += 1

    def densify_and_prune(self, max_grad, min_opacity, prune_big_points, exclude_list=[], \
            min_opacity_bkgd=None, bkgd_index=None, egopose_index=None, judge_huge_obj=False):
        scalars = {}
        tensors = {}
        if min_opacity_bkgd is None:
            min_opacity_bkgd = min_opacity
        ...
        for model_name in self.model_name_id.keys():
            ...
            model: GaussianModel = getattr(self, model_name)
            if model_name in ['background', 'ground']:  
                scalars_, tensors_ = model.densify_and_prune(max_grad, min_opacity_bkgd, prune_big_points, bkgd_index, egopose_index)
                scalars.update({f"{model_name}_{k}": v for k, v in scalars_.items()})
            else:
                if model.get_xyz.shape[0] > 1:
                    if not judge_huge_obj:
                        model.densify_and_prune(max_grad, min_opacity, prune_big_points, bkgd_index, egopose_index)
                    else:
                        if model.bbox[0] > cfg.model.gaussian.huge_vehicle_length:
                            model.densify_and_prune(max_grad, min_opacity, prune_big_points, bkgd_index, egopose_index)
    ...
    def get_number_of_gaussians(self):
        ...
        for model_name in self.model_name_id.keys():
            model: GaussianModel = getattr(self, model_name)
            try:
                scalar_dict[f"number_of_{model_name}"] = model.get_xyz.shape[0]
            except Exception as e:
                pass
        return scalar_dict
    ...
    def reset_opacity(self, exclude_list=[]):
        for model_name in self.model_name_id.keys():
            model: GaussianModel = getattr(self, model_name)
            if startswith_any(model_name, exclude_list) or model.get_xyz.shape[0] < 2:
                continue
            model.reset_opacity()
