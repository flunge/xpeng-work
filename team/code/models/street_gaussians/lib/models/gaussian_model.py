...
from lib.models.appearance_network import AppearanceNetwork
...
class GaussianModel(nn.Module):
    def __init__(self, model_name='background', num_classes=1):
        ...
        if self.model_name in ['background', 'ground']:
        ...
        self.optimizer = None
        self.optimizer_phase2 = None
        self.percent_dense = 0
        self.spatial_lr_scale = 0
        
        std = 1e-4
        self._appearance_embeddings = torch.nn.Parameter(torch.empty(4096, 64).to("cuda"))
    ...
    def make_ply_visualization(self):
        xyz = self._xyz.detach().cpu().numpy()
        opacities = self.get_opacity.detach().cpu().numpy()
        scale = self.get_scaling.detach().cpu().numpy()
        rotations_mat = quaternion_to_matrix(self.get_rotation).detach().cpu().numpy()
        roll = np.arctan2(rotations_mat[:, 2, 1], rotations_mat[:, 2, 2])[:, None]
        pitch = np.arctan2(
            -rotations_mat[:, 2, 0], 
            np.sqrt(rotations_mat[:, 2, 1]**2 + rotations_mat[:, 2, 2]**2)
        )[:, None]
        names = ['x', 'y', 'z', 'opacity', 'sx', 'sy', 'sz', 'roll', 'pitch']
        dtype_full = [(attribute, 'f4') for attribute in names]

        elements = np.empty(xyz.shape[0], dtype=dtype_full)
        attributes = np.concatenate((xyz, opacities, scale, roll, pitch), axis=1)
        elements[:] = list(map(tuple, attributes))
    ...
    def load_state_dict(self, state_dict):  
        ...
        if cfg.mode == 'train':
            ...
            if 'optimizer_phase2' in state_dict:
                self.optimizer_phase2.load_state_dict(state_dict['optimizer_phase2'])
            if 'appearance_embeddings' in state_dict:
                self._appearance_embeddings = state_dict['appearance_embeddings']
            if 'appearance_network' in state_dict:
    ...
    def state_dict(self, is_final=False):
        state_dict = {
            ...
            'appearance_embeddings': self._appearance_embeddings,
        ...
        if not is_final:
            ...
            if self.optimizer_phase2 is not None:
                state_dict_extra.update({
    ...
    def maxscale_loss(self, maxsize=0.8):
        scales = self.get_scaling
        sx = scales[:, 0]
        sy = scales[:, 1]   
        sz = scales[:, 2]
    ...
    def training_setup_phase2(self):
        args = cfg.optim
    ...
    def update_optimizer_phase2(self):
        self.optimizer_phase2.step()
        self.optimizer_phase2.zero_grad(set_to_none=True)
    ...
    def prune_points(self, mask, prune_list=None):
        ...
        if cfg.mode == 'train':
            ...
            self._semantic = nn.Parameter(self._semantic[valid_points_mask].requires_grad_(True))
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2, init_lidar_pts_mask=None):
        n_init_points = self.get_xyz.shape[0]
                
        # 可以认作是grads，即每个高斯球的位置梯度
        padded_grad = torch.zeros((n_init_points), device="cuda")
        padded_grad[:grads.shape[0]] = grads.squeeze()
        # shape依然和grads相同，但是每个元素的值为scene_extent
        ...
        if init_lidar_pts_mask != None:
            selected_pts_mask =  torch.logical_and(selected_pts_mask, init_lidar_pts_mask.cuda())

        self.scalar_dict['points_split'] = selected_pts_mask.sum().item()

        ## 下面三行代码做的事：在原始点范围（scaling）内，以高斯分布随机撒N个点，sample为新点相对于原始点的位置 ##
        # 选中需要分裂的点的scaling，复制N份。例如选中了10个点，那么复制N份，每一份都和原始10个点的scale相同
        stds = self.get_scaling[selected_pts_mask].repeat(N, 1)
        means = torch.zeros((stds.size(0), 3), device="cuda")
        samples = torch.normal(mean=means, std=stds)
        rots = quaternion_to_matrix(self._rotation[selected_pts_mask]).repeat(N, 1, 1)
        # 新点使用原始点的rotation（高斯球旋转），并转换为世界坐标系
        ...
        # selected_pts_mask为需要分裂的原始点，N * selected_pts_mask.sum()为分裂出来的点数量
        # prune_filter中，需要分裂的原始点被置为true，其余原始点为false，新分裂出来的点为0 / false
        prune_filter = torch.cat((selected_pts_mask, torch.zeros(N * selected_pts_mask.sum(), device="cuda", dtype=bool)))
        # 删除被分裂的原始点
        self.prune_points(prune_filter)

    def densify_and_clone(self, grads, grad_threshold, scene_extent, init_lidar_pts_mask=None):
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, bkgd_index=None, egopose_index=None):
        grads = self.xyz_gradient_accum[:, 0:1] / self.denom
        grads[grads.isnan()] = 0.0
        
        self.scalar_dict.clear()
        self.scalar_dict['points_total'] = self.get_xyz.shape[0]
        print(f'[BASE] Number of current gaussians: {self.get_xyz.shape[0]}')
        ...
        print(f'[BASE] Number of pruned gaussians: {prune_mask.sum()} due to size and opacity')
