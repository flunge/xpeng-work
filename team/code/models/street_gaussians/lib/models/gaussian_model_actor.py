...
class GaussianModelActor(GaussianModel):
    def __init__(
        ...
        # record fourier and feature dc values
        self.fouriered_dc_list = dict()
        self.color_list = dict()
        self.origin_feature_dc_values = list()
    ...
    def get_features_fourier(self, frame=0, obj_name=""):
        ...
        # debug_name = "obj_000000999"
        # if obj_name == debug_name:
        #     # print feature dc min and max value
        #     # print(f'Before: Fourier scale: {self.fourier_scale}, Feature DC min: {features_dc.min().item()}, max: {features_dc.max().item()}, shape: {features_dc.shape}, C dim: {features_dc.shape[1]}')
        #     features_dc_orig = torch.abs(torch.sum(features_dc.squeeze(), dim=1))
        #     if len(features_dc_orig.shape) > 1 and features_dc_orig.shape[1] > 1:
        #         dim1_shape = features_dc_orig.shape[1]
        #         features_dc_orig = features_dc_orig / dim1_shape
        #         features_dc_orig = torch.sum(features_dc_orig, dim=1)

        #     self.origin_feature_dc_values = features_dc_orig.cpu().numpy().tolist()
        #     # print(f'Feature DC mean: {np.mean(self.origin_feature_dc_values)}')

        features_dc = torch.sum(features_dc * idft_base[..., None], dim=1, keepdim=True) # [N, 1, 3]
        
        # if obj_name == debug_name:
        #     # print(f'After: Fourier scale: {self.fourier_scale}, Feature DC min: {features_dc.min().item()}, max: {features_dc.max().item()}, shape: {features_dc.shape}, C dim: {features_dc.shape[1]}')
        #     self.fouriered_dc_list[frame] = {
        #         "min_feature": features_dc.min().item(),
        #         "max_feature": features_dc.max().item()
        #     }
    ...
    def densify_and_prune(self, max_grad, min_opacity, prune_big_points, bkgd_index=None, egopose_index=None):
        ...
        # Prune large points on pixel space
        max_gs_size = 0.8 * cfg.optim.get('max_screen_size', 250)
        big_points_vs = self.max_radii2D > max_gs_size
