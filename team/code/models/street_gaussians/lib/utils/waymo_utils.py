...
def generate_dataparser_outputs(
    ...
    for image_filename in image_filenames_all:
        ...
        if frame >= start_frame and frame <= end_frame and cam in cameras:
            ...
            pose_frame = ego_frame_poses[frame]
            c2w = pose @ ext    ########### ext = cam2ego
    ...
    poses_frame = np.stack(poses_frame, axis=0)
    ...
    objects_tracklets_world, object_tracklets_vehicle, object_info = get_obj_pose_tracking(
        datadir, 
        selected_frames, 
        ego_frame_poses,
        cameras,
    ...
    result['obj_tracklets_world'] = objects_tracklets_world
    ...
    if build_pointcloud:
        ...
        for track_id in object_info.keys():
            points_xyz_dict[f'obj_{track_id:09d}'] = []
            points_rgb_dict[f'obj_{track_id:09d}'] = []
        ...
        for i, frame in tqdm(enumerate(range(start_frame, end_frame+1))):
            ...
            for tracklet in object_tracklets_vehicle[i]:
                ...
                if track_id >= 0:
                    ...
                    points_xyz_dict[f'obj_{track_id:09d}'].append(points_xyz_obj[points_xyz_inbbox])
                    points_rgb_dict[f'obj_{track_id:09d}'].append(points_rgb[points_xyz_inbbox])
