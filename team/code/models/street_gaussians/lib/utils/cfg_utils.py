...
def parse_cfg(cfg, args = None):
    ...
    if cfg.record_dir is None:
        ...
        cfg.record_dir = os.path.join(cfg.record_dir, cfg.task, cfg.exp_name, 'record')
...
def reload_cfg(cfg, config_file):
    def merge_cfg(cfg_file, cfg):
        with open(cfg_file, 'r') as f:
            current_cfg = yacs.load_cfg(f)
        if 'parent_cfg' in current_cfg.keys():
        ...
        print(cfg_file)
        return cfg
    cfg_ = merge_cfg(config_file, cfg)
    parse_cfg(cfg_)
    return cfg_
...
def save_cfg(cfg, model_dir, epoch=0):
    ...
    # 处理sim配置，在12347同时存在的时候删除cam2和对应的内外参
    cfg_path = os.path.join(cfg_dir, 'config_sim.yaml')
    with open(cfg_path, 'w') as f:
        # 创建配置副本
        sim_config = copy.deepcopy(cfg)
        
        # 修改相机配置（假设cameras在data属性下）
        if hasattr(sim_config, 'data') and getattr(sim_config.data, 'cameras', None) == [1, 2, 3, 4, 7]:
            # 删除camera 2
            delete_idx = sim_config.data.cameras.index(2)
            sim_config.data.cameras.pop(delete_idx)
            
            # 同步修改相关参数（添加存在性检查）
            if hasattr(sim_config, 'results'):
                if hasattr(sim_config.results, 'extrinsics'):
                    sim_config.results.extrinsics = [
                        cfg.results.extrinsics[0],
                        cfg.results.extrinsics[2],
                        cfg.results.extrinsics[3],
                        cfg.results.extrinsics[4]
                    ]
                
                if hasattr(sim_config.results, 'intrinsics'):
                    sim_config.results.intrinsics = [
                        cfg.results.intrinsics[0],
                        cfg.results.intrinsics[2],
                        cfg.results.intrinsics[3],
                        cfg.results.intrinsics[4]
                    ]
        
        # 写入修改后的配置
        with redirect_stdout(f): print(sim_config.dump())
