...
def fetchGroundSurfelPly(path):
    ...
    scales = np.zeros((positions.shape[0], 3))
    opacities = np.zeros((positions.shape[0], 1))
    return BasicPointCloud(points=positions, colors=colors, normals=normals, rots=rotations, scales=scales, opacities=opacities)

def fetchG3RPly(path):
    plydata = PlyData.read(path)        
    vertices = plydata['vertex']
