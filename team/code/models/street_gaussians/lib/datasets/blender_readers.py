...
def readNerfSyntheticInfo(path, eval, extension=".png", **kwargs):
    ...
    if not os.path.exists(ply_path):
        ...
        rotations = np.zeros((xyz.shape[0], 4))
        rotations[:, 0] = 1 

        scales = np.zeros((xyz.shape[0], 3))
        opacities = np.zeros((xyz.shape[0], 1))

        pcd = BasicPointCloud(points=xyz, colors=SH2RGB(shs), normals=np.zeros((num_pts, 3)), rots=rotations, scales=scales, opacities=opacities)
