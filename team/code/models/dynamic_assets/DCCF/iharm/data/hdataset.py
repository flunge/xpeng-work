...
class HDataset(BaseHDataset):
    def __init__(self, dataset_path, split, blur_target=False, **kwargs):
        ...
        self.dataset_samples = [f.name for f in self._composite_images_path.iterdir() if f.is_file()]
    ...
    def get_sample(self, index):
        ...
        real_image_name = composite_image_name
        mask_name = composite_image_name
        # real_image_name = composite_image_name[:-(len(composite_image_name.split('_')[-1]) + len(composite_image_name.split('_')[-2]) + 2)] + '.png'
...
class HDatasetUpsample(BaseHDatasetUpsample):
    def __init__(self, dataset_path, split, blur_target=False, mini_val=False, **kwargs):
        ...
        print('Using HDatasetUpsample dataset_path:', dataset_path)
        self.dataset_path = Path(dataset_path)
        self.blur_target = blur_target
        self._split = split
        self._real_images_path = self.dataset_path / 'real_images'
    ...
    def get_sample_1(self, index):
        ...
        real_image_name = composite_image_name
        mask_name = composite_image_name
        # real_image_name = composite_image_name.split('_')[0] + '.png'
        # mask_name = '_'.join(composite_image_name.split('_')[:-1]) + '.png'
