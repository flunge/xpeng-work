...
def evaluate_dataset_upsample_hsl_refine(dataset, predictor, metrics_hub_lowres, metrics_hub_fullres, visdir=None):
    ...
    for sample_i in trange(len(dataset), desc=f'Testing on {metrics_hub_lowres.name}'):
        ...
        print(f"shapes: raw_image {raw_image.shape}, raw_target {raw_target.shape}, raw_mask {raw_mask.shape}")
        ...
        pred, pred_fullres, inter_result = predictor.predict(sample['image'], image_fullres, raw_image, sample['target_image'], raw_target, 
                sample_mask, sample_mask_fullres, raw_mask, return_numpy=False)
        ...
        if visdir and sample_i % 1 == 0:
            ...
            # 保存原始分辨率（未降采样）预测结果
            # pred_fullres_np = pred_fullres.cpu().numpy()
            cv2.imwrite(
                os.path.join(visdir, f'{imname}.png'), 
                pred_fullres[:, :, ::-1]  # 转换BGR为RGB
            )
...
#         raw_target = bdata['target_image']
#         raw_mask = bdata['object_mask']
#         sample = dataset.augment_sample(bdata, dataset.augmentator_2)  # 低分辨率样本
#         sample_fullres = dataset.augment_sample(bdata, dataset.augmentator_1)  # 高分辨率样本
        
#         # 模型预测（同时得到低分辨率和高分辨率结果）
#         predict_start = time()
#         pred, pred_fullres, inter_result = predictor.predict(
#             sample['image'], image_fullres, raw_image, 
#             sample['target_image'], raw_target, 
#             sample['object_mask'], sample_fullres['object_mask'], 
#             return_numpy=False
#         )
#         torch.cuda.synchronize()
#         metrics_hub_lowres.update_time(time() - predict_start)
...
#         with torch.no_grad():
#             target_image = torch.as_tensor(sample['target_image'], dtype=torch.float32).to(predictor.device)
#             sample_mask = torch.as_tensor(sample['object_mask'], dtype=torch.float32).to(predictor.device)
#             metrics_hub_lowres.compute_and_add(pred, target_image, sample_mask)
...
#             # pred = pred.cpu().numpy()
#             # cv2.imwrite(
#             #     os.path.join(visdir, f'{imname}_pred_lowres.jpg'), 
#             #     pred[:, :, ::-1]  # 转换BGR为RGB
#             # )
...
#             pred_fullres_np = pred_fullres.cpu().numpy()
#             cv2.imwrite(
#                 os.path.join(visdir, f'{imname}_pred_fullres.jpg'), 
#                 pred_fullres_np[:, :, ::-1]  # 转换BGR为RGB
#             )

#             # # 可选：同时保存原始输入和目标图像（原始分辨率）
#             # cv2.imwrite(
#             #     os.path.join(visdir, f'{imname}_input_fullres.jpg'), 
#             #     raw_image[:, :, ::-1]
#             # )
#             # cv2.imwrite(
#             #     os.path.join(visdir, f'{imname}_gt_fullres.jpg'), 
#             #     bdata['target_image'][:, :, ::-1]
#             # )
