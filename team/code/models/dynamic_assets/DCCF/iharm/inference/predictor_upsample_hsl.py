...
print(f"import root_path {root_path}")
...
class PredictorUpsampleHSL(object):
    def __init__(self, net, device, with_flip=False,
        ...
        self.avgs = []
        self.smoother = CameraFilterSmoother()

    def predict(self, image, image_fullres, raw_image, target_image, target_image_fullres, mask, mask_fullres, origin_raw_mask, return_numpy=True):
        rc = False
        with torch.no_grad():
            raw_mask = mask
            ...
            filters_fullres, refine_filters_fullres = self.smoother.smooth(
                current_filters=filters_fullres,
                current_refines=refine_filters_fullres,
                raw_mask=to_tensor_and_normalize(origin_raw_mask).to(self.device),
                smooth_channels='HSV',
            )
            ...
            if self.net.model.use_attn:
                ...
                # mask_tensor = to_tensor_and_normalize(origin_raw_mask).to(self.device)
                # # soft_mask = gaussian_blur(mask_tensor, kernel_size=5, sigma=1.0) 
                # output_fullres = output_fullres * mask_tensor + raw_image * (1 - mask_tensor)
    ...
    def display_smooth_plot(self, saved_dir):
        self.smoother.display_channel_avgs_plot(saved_dir=saved_dir)

def to_tensor_and_normalize(mask):
    ...
    if mask.ndim == 2:
        ...
        if mask.shape[0] == 1 or mask.shape[-1] == 1:
            ...
            mask = mask.permute(2, 0, 1).unsqueeze(0)  # assume HWC to BCHW
    return mask.float()