...
class LiGSToolkit:
    ...
    def process_images(self, to_copy=False):
        ...
        if len(os.listdir(save_image_dir)) <= 0:
            if not to_copy:
                ...
                os.system(f'cp {os.path.abspath(self._dataset_dir)}/images/* {os.path.abspath(save_image_dir)}/')
            logging.info(f'Link images into {save_image_dir}')
        else:
            logging.info(f'Skipped linking/Copying images since exists!')

    def process_alphas(self, to_copy=False):
        ...
        if len(os.listdir(save_alpha_dir)) <= 0:
            if not to_copy:
                ...
                os.system(f'cp {os.path.abspath(self._alphas_dir)}/* {os.path.abspath(save_alpha_dir)}/')
            logging.info(f'Link masks into {save_alpha_dir}')
        else:
            logging.info(f'Skipped linking alphas since exists!')

    def process_arkit(self, to_copy=False):
        ...
        if os.path.exists(self._dataset_dir + '/arkit'):
            ...
            if len(os.listdir(save_arkit_dir)) <= 0:
                if not to_copy:
                    ...
                    os.system(f'cp {os.path.abspath(self._dataset_dir)}/arkit/* {os.path.abspath(save_arkit_dir)}/')
    ...
    def save_dataset(self, pcd, colors, cam_intrinsics, cam_extrinsics, inds=None, to_copy=False):
        ...
        self.process_images(to_copy)
        self.process_alphas(to_copy)
        self.process_arkit(to_copy)
