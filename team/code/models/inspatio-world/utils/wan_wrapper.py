...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, model_folder: str) -> None:
        ...
        self.text_encoder = umt5_xxl(
            ...
            # The checkpoint is bf16; matching it avoids an unnecessary fp32
            # expansion during CPU-side initialization.
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        if weight_list:
            model_path = os.path.join(ckpt, weight_list[0]['path'])
        else:
        ...
        if rank == 0:
            ...
            if ckpt_path is not None:
                ...
                for weight_config in weight_list:
                    weight_path = os.path.join(ckpt, weight_config['path'])
