...
def load_clip_to_cpu(backbone_name):
    ...
    # 优先从共享目录加载，避免联网下载
    _shared_clip_dir = "/workspace/group_share/adc-sim/users/cloudsim/clip"
    _filename = os.path.basename(url)
    _shared_path = os.path.join(_shared_clip_dir, _filename)
    if os.path.isfile(_shared_path):
        model_path = _shared_path
        print(f"Loading CLIP model from shared directory: {model_path}")
    else:
