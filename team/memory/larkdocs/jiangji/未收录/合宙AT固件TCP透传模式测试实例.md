<title>合宙AT固件TCP透传模式测试实例</title>

# 合宙AT固件TCP透传模式测试实例

> 来源: https://yinerda.yuque.com/yt1fh6/4gdtu/rgoqkbpsp2d0ugpx  
> 路径: 合宙模组系列AT固件使用方法 > 合宙AT固件TCP透传模式测试实例
...
|  |  |  |
| 11 | 创建TCP连接，支持IP和域名。用域名直接替换IP即可。连接成功后会返回CONNECT OK 可以用自己的服务器，替换IP和端口，测试的时候可以用银尔达测试服务器获取IP 浏览器打开[http://test.yinerda.com](http://test.yinerda.com)，选择“TCP测试工具”，点击"打开"可以获取测试服务器信息 |  |
|  |  |  |
