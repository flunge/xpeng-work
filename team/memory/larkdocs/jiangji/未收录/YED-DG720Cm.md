<title>YED-DG720Cm</title>

# YED-DG720Cm

> 来源: https://yinerda.yuque.com/yt1fh6/4gdtu/qkcp3fqvohszvr5g  
> 路径: 产品用户手册 > 4G DTU > YED-DG720Cm

# 简介

YED-DG720Cm DTU是由银尔达（yinerda）推出的工业级的单TTL串口DTU 。小巧、稳定、可靠。适合设备控制，状态检测，传感器数据采集等通过4G网络与服务器通讯的场景，特性如下:

1. 支持直流5\~36V宽电压供电；
2. 支持标准35mm导轨安装和螺丝孔安装，外壳阻燃材料；
3. 支持接触放电±8KV，空气放电±15KV；
4. 工作环境为-35℃-75℃；
5. 支持1路TTL串口，兼容3.3V和5V串口电平；
6. 支持供电电源电压采集；
...
| 工作环境  | 工作温度 | -35℃ \~+75℃ |
| 工作湿度 | 5%\~95%RH(无凝露) |  |
| TTL串口 | 1路 | 1200-460800；数据位:8、7 ；停止位：1、2；校验位：奇、偶、无校验 兼容3.3V和5V串口电平 |
...
| TX RX GND |  | TTL串口，兼容3.3V 和5V串口电平 |  |
...
模块的VIN GND 接5\~36V,10W的电源(不能用USB转串口那种电脑USB供电，功率不足)；TTL串口 TX接RX，RX接TX,GND 接GND；推荐使用银尔达的YED-UUART-211测试工具测试，方便供电和串口连接调试。
