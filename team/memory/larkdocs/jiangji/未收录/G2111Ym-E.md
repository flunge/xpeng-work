<title>G2111Ym-E</title>

# G2111Ym-E

> 来源: https://yinerda.yuque.com/yt1fh6/4gdtu/dekuds5w2kthhdw7  
> 路径: 产品用户手册 > 定位模块 > G2111Ym-E

# 简介

G2111Y-E 核心板是由银尔达（yinerda）推出的工业级的单串口DTU 。小巧、稳定、可靠。适合设备控制，状态检测，传感器数据采集等通过4G网络与服务器通讯的场景。
...
2. 工作环境为-35℃-75℃；
3. 支持外部独立硬件看门狗，不死机；
4. 支持1路高电平输入，检测5\~90V高电平输入；
5. 支持1路NPN输出，最高支持24V电压输出；
6. 支持1路RS485串口；
7. 支持独立定位模块ATGM336H-5NR32(GPS+北斗)或ATGM336H-6N22(单北斗)；
8. 支持基站定位/WIFI定位(大陆)；
9. 支持AGNSS辅助定位，加快定位速度；
10. 支持1路ADC，采集供电电压；
11. 支持运动震动检测；
...
|-|-|-|-|
| 编号 | 标识 | 功能 | 说明 |
| 1 | VIN GND | 电源正 电源负 | VIN表示正极，GND表示负极 供电范围：8\~90V（10W） |
| 2 | IN1 |  | 5\~90V高电平检测输入 |
| 3 | OUT |  | NPN输出，支持最高24V电压控制 |
| 4 | A B | RS485串口 |  |
| 5 | 外置SIM卡 |  | 翻盖小卡 |
| 6 | NET LED |  | 系统指示灯 |
| 7 | 1PPSLED | GPS指示灯 | GPS定位成功 1s闪烁 |
| 8 | 4G | 4G天线 | SMA螺旋内针 |
| 9 | GPS | GPS天线 | SMA螺旋内针 |
| 10 | USB | 固件升级或者调试 | USB |
| 11 | BOOT |  | 与USB配合用来升级 |
| 12 | Reload |  | 长按7秒，恢复出厂设置 |
| 13 | 贴片SIM卡 |  | 贴片SIM卡稳定，不松动，脱落，整个板子可以喷三防漆，缺点是不能跟换卡 |
| 14 | GPS模块 |  | 可选GPS模块ATGM336H-5NR32或ATGM336H-6N22单北斗芯片 |
