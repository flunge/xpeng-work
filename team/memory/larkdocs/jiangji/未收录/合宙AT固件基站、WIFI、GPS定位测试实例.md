<title>合宙AT固件基站、WIFI、GPS定位测试实例</title>

# 合宙AT固件基站、WIFI、GPS定位测试实例

> 来源: https://yinerda.yuque.com/yt1fh6/4gdtu/syk96zahx6fungdm  
> 路径: 合宙模组系列AT固件使用方法 > 合宙AT固件基站、WIFI、GPS定位测试实例

AT固件不支持RS485发送，支持TTL，RS232等发送。

本文档目前是Air820和Air780EG 支持定位功能的模块支持。
...
串口工具你可以选择任意工具，这里银尔达提供格西烽火和测试工程，推荐使用。

注意基站定位和WIFI定位，可以在室内定位，有差距，也可能定位不成功(数据库没数据)，中国大陆使用。

注意GPS定位，只能室外用，精确，可以全球用。
...
|  |  |  |
| 10 | 基站定位，基站定位需要网络正常才能用，最长超时60秒才会返回，等到超时后没返回可以认为异常 |  |
|  |  |  |
| 11 | WIFI定位，WIFI定位需要网络正常才能用，最长超时60秒才会返回，等到超时后没返回可以认为异常 |  |
|  |  |  |
| 12 | 打开gps，设置辅助定位 |  |
|  |  |  |
| 14 | 读取GPS信息，如果没数据就没定位成功，可以周期查询，返回的数据参考AT手册。 如果120都没定位成功，基本就可以说明GPS异常或者GPS没信号，可以做异常处理。 通过第2位数据，如果是0表示定位失败，1表示定位成功。 |  |
|  |  |  |
| 15 | 开启NMEA 数据输出到 AT串口，这种是不必须的用来调试用的 |  |
|  |  |  |
