<title>UDP配置测试实例</title>

# UDP配置测试实例

> 来源: https://yinerda.yuque.com/yt1fh6/4gdtu/sgoxuutg6gemmxvs  
> 路径: DTU固件实例讲解 > UDP配置测试实例
...
**注意:UDP是非连接的，必须DTU给服务器先发数据，服务器才能发给DTU，如果所以最好配置注册包**

**注意:UDP是非连接的，在识别设备身份的时候，建议使用前置数据识别DTU身份。**

## 3.1、获取测试服务器地址和端口

打开DTU测试平台:[http://test.yinerda.com](http://test.yinerda.com)，选择“UDP测试工具”，点击"打开"，可以获取到UDP测试IP地址/域名 和端口号。 IP地址或者域名就是UDP的服务器地址，任选其一。

注意:浏览器工具只是用来测试和验证设备使用。10分钟没有任何交互会自动关闭服务器，如果发现数据不通了，重新刷新浏览器，重新打开，获取新资源测试。

## 3.2、配置参数

在“网络通道参数”界面配置UDP协议的参数。前置数据一般用来识别UDP设备身份。
...
注意:UDP必须设备连接服务器，给服务器通讯了，服务器才能发数据给DTU。如果中途断开了，也需要DTU首先给服务器发数据才行。

## 3.6、口透传数据到服务器

串口数据一般是字符串和HEX 2种，在测试服务器上面可以点击HEX模式看原始数据。DTU一般可以加前置数据，用来设备身份识别。
