<title>合宙AT固件HTTP GET 测试实例</title>

# 合宙AT固件HTTP GET 测试实例

> 来源: https://yinerda.yuque.com/yt1fh6/4gdtu/emg6rocmz4230zob  
> 路径: 合宙模组系列AT固件使用方法 > 合宙AT固件HTTP GET 测试实例
...
|  |  |  |
| 12 | 设置HTTP参数连接，支持IP和域名。 浏览器打开[http://test.yinerda.com](http://test.yinerda.com)，选择“HTTP测试工具”，点击"打开"可以获取测试服务器信息 注意如果请求有参数url后面要有?号，?号后面根据的请求参数。如果没有参数，就不需要问号 测试平台可是设置服务器返回数据。 |  |
|  |  |  |
| 13 | 关闭HTTP，在关闭之前，之前的参数一直有效，关闭后需要重新初始化 如果HTTP异常可以关闭，但是不必须。如果没关闭，后续只需要提交请求即可 |  |
|  |  |  |
