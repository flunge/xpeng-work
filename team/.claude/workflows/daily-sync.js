...
// 默认目录改为相对脚本定位（team/.claude/workflows/ → team/memory），可用 args 或环境变量覆盖
const REPO_MEMORY = new URL('../../memory', import.meta.url).pathname
const SYNC_DIR = args.syncDir || Deno.env.get('SYNC_DIR') || `${REPO_MEMORY}/daily-sync`
const MEMORY_DIR = args.memoryDir || Deno.env.get('MEMORY_DIR') || REPO_MEMORY
