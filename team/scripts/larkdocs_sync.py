...
WIKI_MANIFEST = MIRROR_ROOT / "wiki_manifest.json"   # wiki 嵌套树 auto-generated
...
# wiki 嵌套树根（每次同步前先摸清树 → 刷新 wiki_manifest.json）
WIKI_ROOTS = [
    {"node": "G6I4w06nPiJ1g3kVlJ7cgnZinPg", "space": "7369389337118507009",
     "group": "team", "section": "projects"},
]


def refresh_wiki_manifest(sleep: float = 0.3) -> int:
    """扫 WIKI_ROOTS 存树 → 写 wiki_manifest.json（docx only，bitable 跳过）。"""
    import time as _t
    docs = []
    for root in WIKI_ROOTS:
        nodes, pt = [], None
        while True:
            args = ["wiki", "+node-list", "--space-id", root["space"],
                    "--parent-node-token", root["node"], "--as", "user", "--format", "json"]
            if pt:
                args += ["--page-token", pt]
            d, err = run_cli(args, profile="xpeng")
            if not d:
                print(f"[wiki] node-list 失败 {root['node'][:8]}: {err}"); break
            nodes.extend(((d.get("data") or {}).get("nodes") or []))
            ptk = (d.get("data") or {}).get("page_token")
            if not (d.get("data") or {}).get("has_more") or not ptk or ptk == pt:
                break
            pt = ptk
            _t.sleep(sleep)
        for n in nodes:
            if n.get("obj_type") != "docx" or not n.get("obj_token"):
                continue
            title = n.get("title") or n["obj_token"][:12]
            docs.append({
                "group": root["group"],
                "name": title,
                "path": f"{root['section']}/{title}",
                "rel_path": f"{root['group']}/{root['section']}/{title}.md",
                "token": n["obj_token"],
                "url": f"https://xiaopeng.feishu.cn/docx/{n['obj_token']}",
                "profile": "xpeng",
            })
        _t.sleep(sleep)
    manifest = {
        "_comment": "wiki 嵌套树 manifest（auto-generated by larkdocs_sync.refresh_wiki_manifest）；禁止手工编辑",
        "roots": [r["node"] for r in WIKI_ROOTS],
        "docs": docs,
    }
    WIKI_MANIFEST.write_text(json.dumps(manifest, ensure_ascii=False, indent=1), encoding="utf-8")
    print(f"[wiki] 嵌套树刷新：{len(docs)} 篇 → {WIKI_MANIFEST}")
    return len(docs)


def load_wiki_entries():
    """wiki 嵌套树条目（team/projects 下按 rel_path 对齐）。"""
    entries = []
    if WIKI_MANIFEST.exists():
        try:
            m = json.loads(WIKI_MANIFEST.read_text(encoding="utf-8"))
            for d in m.get("docs", []):
                if d.get("token"):
                    entries.append(d)
        except Exception as ex:
            print(f"[larkdocs-sync] wiki_manifest 读取失败: {ex}")
    return entries
...
def load_all_entries(discover=True):
    ...
    entries = jiangji + team + load_wiki_entries()
    if discover:
...
def main():
    ...
    # wiki 嵌套树刷新（dev.step 在条目合并前走没走都可以，都在后面读 manifest）
    if not args.dry_run:
        try:
            refresh_wiki_manifest()
        except Exception as ex:
            print(f"[wiki] 刷新失败（不阻断）: {ex}")
    if args.group:
