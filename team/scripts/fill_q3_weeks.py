#!/usr/bin/env python3
"""Fill Q3 weekly tables (W29-W40) into the 作战表 document."""
...
# Table content for each week: (week_label, date_range, goals, risks)
# Format: { track: (goal_text, risk_text) }
WEEKS = {
    "W29": {
        "date": "7/13-7/17",
        "场景&生产": (
...
def build_table_xml(week_key, data):
    ...
    xml = f'''<table><colgroup><col width="100"/><col width="500"/><col width="500"/><col width="500"/><col width="500"/></colgroup><tbody>'''
    # Row 1: Header
    xml += f'''<tr><td vertical-align="top"><p align="center"><b>{week_key}</b></p></td>'''
    xml += f'''<td vertical-align="top"><p align="center"><b>场景&amp;生产</b></p></td>'''
    xml += f'''<td vertical-align="top"><p align="center"><b>SIL</b></p></td>'''
    xml += f'''<td vertical-align="top"><p align="center"><b>HIL</b></p></td>'''
    xml += f'''<td vertical-align="top"><p align="center"><b>Agents&amp;预研</b></p></td></tr>'''
    # Row 2: 周目标
    xml += f'''<tr><td vertical-align="top"><p align="center"><b>周目标</b></p></td>'''
    ...
    # Row 3: 核心风险
    xml += f'''<tr><td vertical-align="top"><p align="center"><b>核心风险&amp;上下游交互</b></p></td>'''
...
def run_cmd(args):
    ...
    if result.returncode != 0:
        print(f"ERROR: {' '.join(args[:6])}...")
        print(f"  stderr: {result.stderr[:500]}")
        return None
    return result.stdout


def fetch_block_id(keyword):
    """Fetch block ID for a table matching the keyword heading."""
    out = run_cmd([
        "lark-cli", "docs", "+fetch",
        "--api-version", "v2",
        "--doc", DOC_TOKEN,
...
def replace_table(block_id, new_content):
    """Replace a table block with new content."""
    ...
    if result.returncode != 0:
        ...
        return False
    print(f"  Replaced block {block_id}")
    return True


def insert_after(block_id, content):
    ...
    if result.returncode != 0:
        print(f"  INSERT FAILED: {result.stderr[:300]}")
        return False
    print(f"  Inserted after {block_id}")
    return True


def get_heading_block_id(keyword):
    """Get the heading block ID for a week."""
    out = run_cmd([
        "lark-cli", "docs", "+fetch",
        "--api-version", "v2",
        "--doc", DOC_TOKEN,
    ...
    content = data.get("data", {}).get("document", {}).get("content", "")
    import re
    match = re.search(r'<h3 id="([^"]+)"', content)
    if match:
        return match.group(1)
    return None
...
def main():
    # Phase 1: Replace existing W29, W30, W31 tables
    existing_weeks = ["W29", "W30", "W31"]

    for week in existing_weeks:
        print(f"\n{'='*40}")
        print(f"Processing {week} (replace existing)...")

        # Fetch current table block ID
        block_id = fetch_block_id(week)
        if not block_id:
            print(f"  Could not find table for {week}, skipping")
            continue

        print(f"  Found table block: {block_id}")

        # Build new content
        xml = build_table_xml(week, WEEKS[week])

        # Replace
        ok = replace_table(block_id, xml)
        if not ok:
            print(f"  FAILED to replace {week}")
            sys.exit(1)

        time.sleep(1)  # rate limit

    # Phase 2: Append W32-W40 after W31
    # First, find the W31 table block (re-fetch since IDs may have changed)
    print(f"\n{'='*40}")
    print("Phase 2: Appending W32-W40 after W31...")

    # Get the W31 table block ID (after replacement)
    w31_table_id = fetch_block_id("W31")
    if not w31_table_id:
        print("Could not find W31 table after replacement!")
        sys.exit(1)

    print(f"  W31 table block: {w31_table_id}")

    # Insert weeks W32-W40 one by one after the last inserted block
    last_block_id = w31_table_id

    new_weeks = ["W32", "W33", "W34", "W35", "W36", "W37", "W38", "W39", "W40"]
    ...
    for week in new_weeks:
        ...
        # Build: heading + table
        heading_xml = f'<h3>{week}（{data["date"]}）</h3>'
        table_xml = build_table_xml(week, data)
        content = heading_xml + table_xml

        ok = insert_after(last_block_id, content)
        if not ok:
            print(f"  FAILED to insert {week}")
            sys.exit(1)

        time.sleep(1)  # rate limit

        # Re-fetch the new block ID for chaining
        new_table_id = fetch_block_id(week)
        if new_table_id:
            last_block_id = new_table_id
            print(f"  New block ID for {week}: {new_table_id}")
        else:
            print(f"  WARNING: Could not find {week} after insert, using heading search")
            heading_id = get_heading_block_id(week)
            if heading_id:
                last_block_id = heading_id

    print(f"\n{'='*40}")
    print("Done! All W29-W40 tables filled.")
