 ...
 *
 * 数据源全部在飞书，本地只留规则+关联：
 *   规则：/workspace/.claude/team/（rules/gic-report-style.md 等）
 *   飞书关联：/workspace/team/memory/_feishu_map.json（root_folder=W7rqfwqnnlzSfUdEcIGcjcTNnqe）
 *
 * 风格规范（来自 /workspace/.claude/team/rules/gic-report-style.md）：
 *   1. 每节3-5 bullet + 1张图 — 核心信息在图里
 *   2. column/grid 排版，不用段落——刘先明没时间看长文
 *   3. 可参考其他组内容扩充，但限于算法组范围
 *   4. 不用内嵌文档，细节展开在正文
 *   5. 不谈WM竞争
 *
 * 配图流程（见 /workspace/.claude/team/refs/image-agent.md）：
 *   生图(SoCheap) → 存 team/tmp/ → 【贴图】占位 → 用户 /image 手动上传
 *
...
const IMAGE_DIR = Deno.env.get('GIC_IMAGE_DIR') || '/workspace/team/tmp'
...
数据源全在飞书，用 lark-cli（--as user）读过去 14 天（双周窗口）：
1. 仿真核心日会日报 \`docs +fetch --doc Wu6ywIOM6iEucDkmx3hcEgLHnmg --scope outline\` 定位窗口日期段读全。
2. 会议纪要机器人群 \`im +chat-messages-list --chat-id oc_56b10049700694038662e72aa78e35d3 --start "<起>T00:00:00+08:00" --end "<止>T23:59:59+08:00" --page-size 50\` 逐条提 docx token 读全。
3. 各项目 ledger：读 /workspace/team/memory/_feishu_map.json 的 projects 段拿飞书 token，逐个 fetch 提当前状态+窗口进展+风险。
...
读规则与关联（本地）：
- /workspace/.claude/team/rules/gic-report-style.md（GIC 风格规则）
- /workspace/.claude/team/refs/project-context.md（项目背景）、refs/tokens.md（群/人/文档 token）
- /workspace/team/memory/_feishu_map.json（项目/人物→飞书 token 关联）
