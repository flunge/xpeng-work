...
每周五生成。覆盖周期：**上周五 00:00 → 本周五 12:00**。
...
1. **Collect** — 按优先级顺序串行采集（不可跳过任何一步）：
   - **Step 0【必须】仿真核心日会日报**：`https://xiaopeng.feishu.cn/wiki/Wu6ywIOM6iEucDkmx3hcEgLHnmg`（仿真组日报），读时间窗口内每天的「进展」段 + 周进展段（开环/闭环交付数、场景毕业大盘、双周会同步、能力建设）
   - **Step 1【必须】Q2 Wiki/作战表（下午例会日报）**：遍历 `https://xiaopeng.feishu.cn/wiki/SBUYwm8Lri9aJ6kmexFcBAuGnlh` 中本周时间窗口（上周三→本周二）的进展列——Wiki 是表格格式，每一天是一列（如 0609 列），逐列读完；再读所有嵌套链接文档全文
...
### 🔑 核心进展列的子项展开格式（2026-06-24 学习用户模板 Ml9xdq2X 沉淀）

**核心进展列不是扁平的进展列表，而是按"子项"分层展开。** 每个链路（场景&生产/SIL/HIL/Agents）的核心进展单元格结构如下：

```
【链路名】[链路级内嵌文档]
  · 【子项名】[内嵌文档] @负责人
      进展：<把本周+今日揉成一段整体描述>；（可再嵌套 2-3 个细分 bullet）
      月目标Gap：<相对当月目标的差距，必写>
      协同：<平行团队/兄弟组的支持事项，有合作就写>
  · 【子项2】[内嵌文档] @负责人
      进展：…
      月目标Gap：…
```
...
```xml
<p>【HIL链路】<cite doc-id="xxx" ...></cite></p>
<ul>
  <li>【常规模式】<cite doc-id="xxx"></cite><cite type="user" user-id="ou_xxx" user-name="负责人"></cite>
    <ul>
      <li>进展：…；<ul><li>细分点1</li><li>细分点2</li></ul></li>
      <li>月目标Gap：…</li>
      <li>协同：与XX组（负责人）……</li>
    </ul>
  </li>
...
**三类子条目的写法约定**：
- **进展：** 把"本周 + 今日"揉成**一段整体描述**说清进展，**不要每个任务单独写一行 0623/0624 的日期流水**。需要时在其下再嵌 2-3 个细分 bullet。
- **月目标Gap：** 每个子项**必写**，明确"距月目标（引用月目标列的对应目标）还差什么/卡在哪"。
- **协同：** 凡有兄弟组/平行团队支持的子项就加一条，点名**平行团队 + 对接人**（如平台研发组/引擎开发组/评估开发组/AI引擎组/仿真业务组/数据生产组/台架开发组）。**只写平行团队，不写本组成员**（周蔚旭/郑丽娜等是李坤自己组的，不算"协同支持"）。

**子项级别要素**：每个子项标题用 `【子项名】`，紧跟内嵌文档链接（`<cite doc-id=...>`）和 `@负责人`（`<cite type="user">`）。

**周报标题与表头**：文档首行 `# 周报 YYYY-MM-DD`；表头第三列写成带周期的 `核心进展{起}-{止}`（如 `核心进展0617-0623`）。月目标列仍保持【业务交付】【算法优化】等分类标签、不修改。
...
- `user-id` 从哪儿来：会议纪要 doc 的参会人列表（`<cite type="user" ...>`，最可靠）、`memory/people/_index.md` 人员索引、或 `pipelines/gic-report.js` 的 `TEAM_USER_IDS` 映射表
...
```
□ 【硬规则】仿真组日报（Wu6yw）覆盖周期内每天「进展」+周进展段读完
□ 【硬规则】Q2 Wiki/作战表（SBUYwm8）本周所有进展列（逐列）读完，嵌套文档全部打开读全文
...
□ 核心进展列按"子项"展开：【子项】+内嵌文档+@负责人 →（进展 / 月目标Gap / 协同），不是扁平进展列表
□ 每个子项都有「月目标Gap：」；进展是"本周+今日揉成一段"，不是逐日流水
□ 有兄弟组支持的子项加了「协同：」，且只点平行团队+对接人（不写本组成员）
...
### 数据采集
1. **三步铁则，按序执行，不可跳过**：
   - **Step 0 — 仿真组日报**：`https://xiaopeng.feishu.cn/wiki/Wu6ywIOM6iEucDkmx3hcEgLHnmg`，读时间窗口内每天「进展」段 + 周进展段
   - **Step 1 — Q2 Wiki/作战表**：`https://xiaopeng.feishu.cn/wiki/SBUYwm8Lri9aJ6kmexFcBAuGnlh`，读本周时间窗口（上周三→本周二）内所有进展列（表格每列=一天，如 0609 列），再逐一打开每列里所有嵌套链接文档读全文
...
| 核心进展写成扁平流水 | 一个链路一坨进展，没按子项分，没有 Gap | 按【子项】展开：每子项给 进展/月目标Gap/协同 三类条目 | 6/24 |
| 逐日一行写进展 | "↳0623…""↳0624…"每天一行 | 把本周+今日**揉成一段整体描述**，必要时其下再嵌细分 bullet | 6/24 |
| 协同写了本组人 | "协同：周蔚旭…"——他是李坤自己组的 | 协同只点**平行团队+对接人**（平台研发/引擎开发/评估开发/AI引擎/仿真业务/数据生产/台架开发） | 6/24 |
...
- 高炳涛画像：`memory/people/gaobingtao.md`
- 文档规范：`memory/insights/doc-rules.md`
- 纪要规则：`memory/insights/meeting-rules.md`
- 图像Agent：`memory/refs/image-agent.md`
