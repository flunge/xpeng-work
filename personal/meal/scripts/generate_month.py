...
# 数据源：迁飞书后默认从飞书 Base 读配方 / 从飞书拉 config。
# feishu_data 不可用（如本地离线调试）时自动回退本地文件。
try:
    import feishu_data
except Exception:
    feishu_data = None
...
def load_all_recipes(recipe_type):
    ...
    默认从飞书 Base 读取（原始YAML 列无损还原）；飞书不可用时回退本地
    recipes/<type>/*.yaml。
    """
    if feishu_data is not None:
        return feishu_data.load_all_recipes(recipe_type)
    recipes = []
...
def main():
    ...
    parser.add_argument("--no-upload", action="store_true", help="只在本地生成，不同步到飞书")
    args = parser.parse_args()

    # 加载配置（优先飞书，回退本地）
    family = load_config("family.yaml")
    holidays_config = load_config("holidays-2026.yaml")
    holidays = holidays_config.get("holidays", [])
    vacations = []
    try:
        vacations = (load_config("vacations-2026.yaml") or {}).get("vacations", [])
    ...
    # 同步到飞书（月计划 + 每日菜谱），--no-upload 可跳过
    if feishu_data is not None and not args.no_upload:
        try:
            feishu_data.upload_plans(PLANS_DIR)
            feishu_data.upload_daily(DAILY_DIR)
            print("  ☁️ 已同步月计划与每日菜谱到飞书")
        except Exception as e:
            print(f"  ⚠️ 飞书同步失败（本地已生成）：{e}")
