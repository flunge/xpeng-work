...
| 内容 | 位置 | Pod 重启后 |
|---|---|---|
| 工程文件（`/workspace/personal/meal` 全部） | 持久盘 `/dev/nvme1n1` | ✅ 保留 |
...
（如不想自动启动 cron 守护进程：`SKIP_CRON_DAEMON=1 bash /workspace/personal/meal/setup.sh`）
...
```cron
0 18 * * *     /workspace/personal/meal/scripts/run_daily.sh          # 每日 18:00 发送明日食谱
0 20 28-31 * * /workspace/personal/meal/scripts/generate_next_month.sh # 月末 20:00 生成下月计划
0 */2 * * *    /workspace/personal/meal/scripts/check_feedback.sh      # 每 2 小时检查群反馈
```
